...
func setup(ingredient_1: IngredientModel, ingredient_2: IngredientModel, difficulty: float = 1.0, inventory: InventoryManager = null) -> void:
	...
	# More ingredients = more moisture to manage = slower drain
	...
	current_moisture = max_moisture + bonus_moisture_total
	active_drain_modifiers.clear()
	
	# Calculate drain using combined stats (more ingredients = slower drain)
	var base_drain = 5.0
	var drain = base_drain + (total_volatility * 0.25) - (total_heat_resist * 0.02) - (total_density * 0.015)
	base_drain_rate = maxf(drain, 3.0)

	# Penalty: using same ingredient twice increases drain (no synergy)
	var synergy_note = ""
	...
	print("Max moisture: %.2f | Current (with bonus): %.2f | Bonus: %.2f" % [max_moisture, current_moisture, bonus_moisture_total])
	print("==================")
