...
│   ├── main_menu.tscn                  # Main menu with recipe book
│   ├── hand_selector.tscn              # Card selection UI
│   ├── ingredient_card.tscn            # Draggable ingredient card
│   ├── shop.tscn                       # Shop UI (3-sample system)
│   ├── recipe_book.tscn                # Recipe discovery viewer
│   ├── recipe_card_reveal.tscn         # New recipe animation
│   └── recipe_discovery_reveal.tscn    # End-game discoveries screen
├── scripts/
│   ├── main.gd                         # Main game orchestrator
│   ├── models/
│   │   └── ingredient_model.gd         # Ingredient/recipe data structure
│   ├── managers/
│   │   ├── moisture_manager.gd         # Moisture drain calculation
│   │   ├── timer_manager.gd            # Round timer management
│   │   ├── fridge_manager.gd           # Deck/hand management
│   │   ├── shop_manager.gd             # Shop logic (samples, organic)
│   │   ├── progression_manager.gd      # Tier unlocking system
│   │   ├── recipe_book_manager.gd      # Recipe discovery persistence
│   │   ├── inventory_manager.gd        # Relics (passive items)
│   │   └── currency_manager.gd         # Currency tracking
│   ├── ui/
│   │   ├── hand_selector.gd            # Card selection + drag-drop
│   │   ├── ingredient_card.gd          # Card display + interaction
│   │   ├── recipe_book.gd              # Recipe book UI with pagination
│   │   ├── recipe_book_entry.gd        # Individual recipe entry (clickable)
│   │   ├── recipe_card_reveal.gd       # Recipe reveal animation
│   │   └── recipe_discovery_reveal.gd  # End-game discoveries display
│   ├── data/
│   │   ├── ingredients.gd              # Base ingredient data (8 base + 5 premium)
│   │   ├── recipes.gd                  # Recipe combination logic + 8 tier system
│   │   └── relics_data.gd              # Passive item definitions
│   └── singletons/
│       └── event_bus.gd                # Signal hub (autoload)
├── Assets/
│   ├── Food/                           # Ingredient sprites
│   ├── Fonts/                          # Early GameBoy font
...
**Discovery Flow:**
1. **In-Game**: Create a new recipe → Gold notification appears at top
...
- Discovered recipes show name; undiscovered show "🔒 ???"
...
1. Open Godot 4.3+
2. Import project (`File → Import Project`)
