...
func _on_game_started() -> void:
	...
	inventory_manager.reset_inventory()  # Start with no items
	current_round_number = 1
	_update_power_level_display()
	_update_currency_display(currency_manager.get_currency())
	
	# Don't give starting items - player must collect them
	print("[Main] Player starts with no active items - must collect them in game")
	_update_item_buttons()  # Hide all buttons initially
