...
@onready var item_button_1: Button = $UI/Background/Microwave/ActiveItemsContainer/ActiveItemButton_1
...
func _on_game_started() -> void:
	...
	# Give player starting active items
	print("[Main] Adding starting active items...")
	var starting_items = [
		ActiveItem.new(ActiveItem.Type.COVER, "Cover", "Trap steam, reduce drain by 40% for 5s", 8.0),
		ActiveItem.new(ActiveItem.Type.STIR, "Stir", "Add water, restore 20 moisture", 6.0),
		ActiveItem.new(ActiveItem.Type.BLOW, "Blow", "Cool the food, reduce drain by 60% for 3s", 10.0)
	]
	
	for item in starting_items:
		inventory_manager.add_active_item(item)
		print("[Main] Added starting item: %s" % item.name)
