...
# UI Screens
# main_menu is a separate scene (main_menu.tscn), not part of main.tscn
var main_menu: Panel = null
@onready var hand_selector: Control = $UI/HandSelector
# ingredient_selector was removed, HandSelector handles this now
var ingredient_selector: Panel = null
...
# moisture_background doesn't exist in scene
var moisture_background: ColorRect = null
...
# relic_display doesn't exist in scene yet
var relic_display: HBoxContainer = null
...
func _ready():
	...
	# Set audio pitch to match game speed for HTML5 compatibility
	if music_player:
		music_player.pitch_scale = game_speed
		print("[Main] Music pitch_scale set to: %.1fx" % game_speed)
	if beep_player:
		beep_player.pitch_scale = game_speed
	if hum_player:
		hum_player.pitch_scale = game_speed
