...
func _on_draft_card_pressed(item: Dictionary, card: Button) -> void:
	if item in selected_items:
		...
		# Check selection limits based on draft type
		var is_milestone = title_label.text.contains("Milestone")
		var is_initial = title_label.text.contains("Game Start")
		var max_sel = 1 if is_milestone else (3 if is_initial else 2)
		
		# Select (if under limit)
		if selected_items.size() < max_sel:
			# Check for ingredient inventory limit
			if item.category == ItemCategory.INGREDIENT:
				if not inventory_manager.can_add_ingredient():
					# Show ingredient trade dialog
					pending_ingredient = item.data
					_show_ingredient_trade_dialog()
					return
...
## Show ingredient trade dialog
func _show_ingredient_trade_dialog() -> void:
	if not ingredient_trade_dialog:
		info_label.text = "Ingredient inventory full (6/6)!"
		return
	
	var new_ingredient: IngredientModel = pending_ingredient
	
	# Get nodes with null checks
	var trade_label = ingredient_trade_dialog.get_node_or_null("VBoxContainer/TradeLabel")
	var buttons_container = ingredient_trade_dialog.get_node_or_null("VBoxContainer/ItemButtons")
	var cancel_button = ingredient_trade_dialog.get_node_or_null("VBoxContainer/CancelButton")
	
	if not trade_label or not buttons_container or not cancel_button:
		info_label.text = "Trade dialog error!"
		return
	
	trade_label.text = "Trade an ingredient for %s?" % new_ingredient.name
	
	# Clear existing buttons
	for child in buttons_container.get_children():
		child.queue_free()
	
	# Create buttons for each owned ingredient
	var owned_ingredients = inventory_manager.get_ingredients()
	for i in range(owned_ingredients.size()):
		var btn = Button.new()
		btn.text = owned_ingredients[i].name
		btn.custom_minimum_size = Vector2(100, 60)
		btn.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		btn.pressed.connect(_on_trade_ingredient_selected.bind(i))
		buttons_container.add_child(btn)
	
	# Setup cancel button
	if not cancel_button.pressed.is_connected(_on_ingredient_trade_cancelled):
		cancel_button.pressed.connect(_on_ingredient_trade_cancelled)
	
	ingredient_trade_dialog.show()

## Handle trading an ingredient
func _on_trade_ingredient_selected(index: int) -> void:
	# Trade out old ingredient for new one
	inventory_manager.trade_ingredient(index, pending_ingredient)
	
	# Add to selected items
	selected_items.append({
		"category": ItemCategory.INGREDIENT,
		"data": pending_ingredient
	})
	
	pending_ingredient = null
	ingredient_trade_dialog.hide()
	info_label.text = "Ingredient traded successfully!"
	_update_confirm_button()

## Cancel ingredient trade
func _on_ingredient_trade_cancelled() -> void:
	pending_ingredient = null
	ingredient_trade_dialog.hide()
	info_label.text = "Trade cancelled"
