...
# Cache to avoid regenerating recipes every time book opens
static var _recipe_cache: Array[Dictionary] = []
...
## Uses static cache to avoid regenerating on every open
func _build_recipe_list():
	# Use cached recipes if already generated
	if _cache_initialized:
		print("[RecipeBook] Using cached recipes (%d total)" % _recipe_cache.size())
		all_recipes = _recipe_cache.duplicate()
		return
	...
	print("[RecipeBook] Generating all recipe combinations (first time - will cache)...")
	...
	print("[RecipeBook] Total recipes generated: %d in %d ms" % [all_recipes.size(), elapsed])
	
	# Cache the results for future opens
	_recipe_cache = all_recipes.duplicate()
	_cache_initialized = true
	print("[RecipeBook] Recipes cached for future use")
