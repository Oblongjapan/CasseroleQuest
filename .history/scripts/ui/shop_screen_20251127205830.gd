	...
	if currency_manager.get_currency() >= cost:
		if currency_manager.spend_currency(cost):
			if fridge_manager and item.has("ingredient"):
				# Show animation feedback before adding to deck
				await _show_ingredient_added_feedback(item.ingredient, container.get_global_rect().get_center())
...
func _on_hover_take_pressed():
	...
	if fridge_manager and current_hover_item.has("ingredient"):
		# Show animation feedback before adding to deck
		await _show_ingredient_added_feedback(current_hover_item.ingredient, current_hover_container.get_global_rect().get_center())
...
func _show_ingredient_added_feedback(ingredient: IngredientModel, start_position: Vector2) -> void:
	...
	# Position the card at the item's position
	...
	var fridge_position = Vector2(641, 360)
	...
	# Slide to fridge
	tween.tween_property(feedback_card, "global_position", fridge_position - (feedback_card.size / 2), 0.8)
