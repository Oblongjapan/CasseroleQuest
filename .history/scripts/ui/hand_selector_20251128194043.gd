...
func show_hand_selection(fridge: FridgeManager, game_state_mgr: GameStateManager = null, prog_mgr: ProgressionManager = null, after_shop: bool = false) -> void:
	...
	if after_shop:
		# After shop: discard remaining cards, get persistent cards (shop samples), draw to fill to 5
		...
		# Get persistent cards (shop samples only - last card was already saved)
		var persistent_cards = fridge_manager.claim_persistent_hand()
		hand_ingredients.append_array(persistent_cards)
		print("[HandSelector] After shop - added %d persistent cards (shop samples) to hand" % persistent_cards.size())
