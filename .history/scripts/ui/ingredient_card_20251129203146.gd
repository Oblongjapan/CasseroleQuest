...
func setup(ing: IngredientModel, upgrade_desc: String = "") -> void:
	...
	if water_label:
		...
		# Apply green color if this stat was upgraded (do this AFTER setting text)
		if ingredient.get_meta("upgraded_stat", "") == "water":
			water_label.add_theme_color_override("font_color", Color(0.2, 1.0, 0.2))
			print("[IngredientCard] Applied GREEN color to water label")
		else:
			# Clear any color override if not upgraded
			water_label.remove_theme_color_override("font_color")
	else:
	...
	if resist_label:
		...
		# Apply green color if this stat was upgraded (do this AFTER setting text)
		if ingredient.get_meta("upgraded_stat", "") == "heat_resistance":
			resist_label.add_theme_color_override("font_color", Color(0.2, 1.0, 0.2))
			print("[IngredientCard] Applied GREEN color to resist label")
		else:
			# Clear any color override if not upgraded
			resist_label.remove_theme_color_override("font_color")
	else:
	...
	if volatility_label:
		...
		# Apply green color if this stat was upgraded (do this AFTER setting text)
		if ingredient.get_meta("upgraded_stat", "") == "volatility":
			volatility_label.add_theme_color_override("font_color", Color(0.2, 1.0, 0.2))
			print("[IngredientCard] Applied GREEN color to volatility label")
		else:
			# Clear any color override if not upgraded
			volatility_label.remove_theme_color_override("font_color")
	else:
