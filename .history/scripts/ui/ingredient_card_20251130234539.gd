...
@onready var upgrade_label: Label = $CardPanel/VBoxContainer/UpgradeLabel if has_node("CardPanel/VBoxContainer/UpgradeLabel") else null
...
func _ready():
	...
	# Make sprite ignore mouse input so clicks pass through to card
	if food_sprite:
		food_sprite.set_meta("_edit_lock_", false)
