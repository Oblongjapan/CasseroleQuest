...
func show_hand_selection(fridge: FridgeManager, game_state_mgr: GameStateManager = null, after_shop: bool = false) -> void:
	...
	if after_shop:
		# After shop: clear hand and draw 5 new cards
...
func confirm_selection() -> void:
	...
	# Discard ALL cards back to the fridge manager
	# Selected cards will be used for cooking (handled separately)
	# Unselected cards need to go back to discard pile
	var unselected_cards: Array[IngredientModel] = []
	for ingredient in hand_ingredients:
		if ingredient not in selected_ingredients:
			unselected_cards.append(ingredient)
	
	print("[HandSelector] Discarding %d unselected cards back to fridge..." % unselected_cards.size())
	if unselected_cards.size() > 0:
		fridge_manager.discard_cards(unselected_cards)
	
	print("[HandSelector] Selected cards (%d) will be used for cooking" % selected_ingredients.size())
	
	# Clear hand_ingredients as all cards have been accounted for
	hand_ingredients.clear()
	
	print("[HandSelector] Hand cleared - all cards returned to fridge")
