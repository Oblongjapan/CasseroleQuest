...
func _populate_hand() -> void:
	...
	# Create card for each ingredient and position them manually so they spread
	var spacing := 40.0
	var n := hand_ingredients.size()

	# Wait one frame so layout sizes are available (hand_container.rect_size)
	await get_tree().process_frame

	# Ensure we have a valid container size (fallback to 800 width)
	var container_width := hand_container.rect_size.x if hand_container and hand_container.rect_size.x > 0 else 800.0
