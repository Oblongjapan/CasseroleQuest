...
signal ingredient_selected(ingredient: IngredientModel, index: int)  # index 0 or 1
signal ingredient_deselected(index: int)  # index 0 or 1
...
func _toggle_card_selection(ingredient: IngredientModel, card: IngredientCard) -> void:
	if ingredient in selected_ingredients:
		...
		ingredient_deselected.emit(index)
	else:
		# Select (if under limit)
		if selected_ingredients.size() < 2:
			var index = selected_ingredients.size()
			...
			ingredient_deselected.emit(0)
			
			# Select new card
			selected_ingredients.append(ingredient)
			card.set_selected(true)
			card.hide()  # Hide the new card
			ingredient_selected.emit(ingredient, 0)
...
## Deselect ingredient by index (called when overlay is dragged out)
func deselect_by_index(index: int) -> void:
	if index >= selected_ingredients.size():
		return
	
	var ingredient = selected_ingredients[index]
	
	# Find the card and show it again
	for card in card_buttons:
		if card.ingredient == ingredient:
			card.set_selected(false)
			card.show()
			break
...
func confirm_selection() -> void:
	...
	print("[HandSelector] Discarding all %d hand cards to fridge manager..." % hand_ingredients.size())
...
func _update_drain_preview() -> void:
	...
	var estimated_drain = 5.0 + (worst_spice * 0.4) - (best_heat * 0.10)
