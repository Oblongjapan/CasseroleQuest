...
func _on_hover_card_entered():
	pass  # Just being on the card keeps it open

## Handle mouse exiting hover card/button
func _on_hover_card_exited():
	# Close the hover card when mouse leaves
	_destroy_hover_card()

## Handle hover end from food sprite
func _on_shop_item_hover_end():
	# Only close if mouse isn't on the hover card
	if hover_card and not hover_card.get_global_rect().has_point(get_viewport().get_mouse_position()):
		_destroy_hover_card()
...
func _on_hover_buy_pressed():
	...
	# Close hover card immediately after purchase
	_clear_hover_state()
...
func _on_hover_take_pressed():
	...
	# Close hover card immediately after taking
	_clear_hover_state()

## Handle sample hover end - remove preview card
func _on_sample_hover_end():
	# Wait a frame to allow transitions between items
...
## Clear hover state immediately (no checks)
