...
func show_hand_selection(fridge: FridgeManager, game_state_mgr: GameStateManager = null, prog_mgr: ProgressionManager = null, after_shop: bool = false) -> void:
	...
	if after_shop:
		# After shop: discard any remaining non-persistent cards, claim persistent cards, draw to fill to 5
		...
		# Get persistent cards (last card + shop samples)
		var persistent_cards = fridge_manager.claim_persistent_hand()
		hand_ingredients.append_array(persistent_cards)
		print("[HandSelector] After shop - added %d persistent cards to hand" % persistent_cards.size())
		...
		# Between rounds: draw cards to fill hand to 5
		var cards_to_draw = max(0, 5 - hand_ingredients.size())
		if cards_to_draw > 0:
			var new_cards = fridge_manager.draw_cards(cards_to_draw)
			hand_ingredients.append_array(new_cards)
			print("[HandSelector] Between rounds - drew %d cards, hand size now: %d" % [cards_to_draw, hand_ingredients.size()])
		else:
			print("[HandSelector] Between rounds - hand already at %d cards, no draw needed" % hand_ingredients.size())
