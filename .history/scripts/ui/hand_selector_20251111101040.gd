...
func _animate_cards_draw() -> void:
	for i in range(card_buttons.size()):
		...
		card.set_meta("original_position", target_pos)
...
func _snap_card_back(card: Control) -> void:
	var original_pos = card.get_meta("original_position", card.position)
	var tween = create_tween()
	tween.tween_property(card, "position", original_pos, 0.2).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
