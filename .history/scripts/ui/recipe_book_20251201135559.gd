...
## Show a recipe card popup when clicking an entry
func show_recipe_card_popup(ingredient: IngredientModel):
	# Remove any existing popup
	if popup_card and is_instance_valid(popup_card):
	...
	# Create card
	popup_card = IngredientCardScene.instantiate()
	overlay.add_child(popup_card)
	
	await get_tree().process_frame
	
	# Setup card
	popup_card.setup(ingredient)
	
	# Center the card
	popup_card.position = Vector2(
		(size.x - popup_card.size.x) / 2.0,
		(size.y - popup_card.size.y) / 2.0
	)
	
	# Animate in
	popup_card.scale = Vector2(0.5, 0.5)
	...
	tween.tween_property(popup_card, "modulate:a", 1.0, 0.3)
	tween.tween_property(popup_card, "scale", Vector2.ONE, 0.4)
...
func _close_popup_card():
	if not popup_card or not is_instance_valid(popup_card):
		return
	
	var overlay = popup_card.get_parent()
