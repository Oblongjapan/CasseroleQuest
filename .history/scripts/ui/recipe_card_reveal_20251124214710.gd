...
var is_interactive: bool = false  # Prevents instant dismissal
...
func show_reveal(recipe_ingredient: IngredientModel) -> void:
	...
	print("[RecipeCardReveal] card_container: %s" % card_container)
	print("[RecipeCardReveal] Self visible: %s" % visible)
	is_dismissed = false
	is_hovering = false
	...
	print("[RecipeCardReveal] Card created and added to container")
	
	# Wait a frame for _ready to complete
	await get_tree().process_frame
	
	print("[RecipeCardReveal] After process frame, card valid: %s" % is_instance_valid(revealed_card))
	...
	print("[RecipeCardReveal] Card positioned at: %v, scale: %v, alpha: %f" % [revealed_card.position, revealed_card.scale, revealed_card.modulate.a])
	...
	# Show the overlay
	show()
	print("[RecipeCardReveal] After show(), visible: %s" % visible)
...
func _animate_card_spring_in() -> void:
	if not revealed_card:
		print("[RecipeCardReveal] ERROR: revealed_card is null in _animate_card_spring_in!")
		return
	
	print("[RecipeCardReveal] Starting spring animation from pos: %v to: %v" % [revealed_card.position, SPRING_POSITION])
	...
	# Spring to center position (use position, not global_position)
	tween.tween_property(revealed_card, "position", SPRING_POSITION, 0.5)
	
	# Scale up to normal size
	tween.tween_property(revealed_card, "scale", CARD_NORMAL_SCALE, 0.5)
	
	await tween.finished
	print("[RecipeCardReveal] Spring-in animation complete, final pos: %v, alpha: %f" % [revealed_card.position, revealed_card.modulate.a])
...
func _animate_card_dismiss() -> void:
	...
	# Slide to the right (recipe box position) - use position, not global_position
	slide_tween.tween_property(revealed_card, "position", RECIPE_BOX_POSITION, SLIDE_DURATION)
