...
## Open the card selector for organic upgrade (random stat)
func open_for_organic_upgrade(fridge_mgr: FridgeManager) -> void:
	current_upgrade = {"organic": true}  # Special marker for organic upgrade
	fridge_manager = fridge_mgr
	
	# Update description label
	upgrade_desc_label.text = "Make Organic: +Random Stat"
...
func _populate_card_grid() -> void:
	...
	for ingredient in all_cards:
		var container = _create_card_for_ingredient(ingredient)
		card_grid.add_child(container)
		
		# After adding to tree, setup the card (it's the first child of the container)
		var card = container.get_child(0) as IngredientCard
		if card:
...
func _get_upgrade_preview(ingredient: IngredientModel) -> String:
	# Check if this is an organic upgrade (random stat)
	if current_upgrade.has("organic") and current_upgrade.organic:
		return "Random stat will be upgraded!\n(+3 to +8 or -2 to -5 Vol)"
