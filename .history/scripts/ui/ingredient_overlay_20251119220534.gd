...
func set_ingredient(ingredient_name: String, ingredient_textures: Array):
	...
	# Spread them horizontally if there are multiple
	var total_width = 0.0
	var spacing = 20.0  # Spacing between items
	
	# Calculate total width needed
	if ingredient_textures.size() > 1:
		for tex in ingredient_textures:
			if tex:
				total_width += tex.get_width()
		total_width += spacing * (ingredient_textures.size() - 1)
	
	# Place layers spread horizontally
	var current_x = -total_width / 2.0 if ingredient_textures.size() > 1 else 0.0
	...
	for i in range(ingredient_textures.size()):
		...
		if layer_texture:
			...
			# Position horizontally - spread across the plate
			if ingredient_textures.size() > 1:
				layer_sprite.position = Vector2(current_x + layer_texture.get_width() / 2.0, 0.0)
				current_x += layer_texture.get_width() + spacing
			else:
			...
			print("[IngredientOverlay]   Added layer %d at x=%f" % [i, layer_sprite.position.x])
		else:
			print("[IngredientOverlay]   WARNING: Layer %d texture is null" % i)
	
	show()
	print("[IngredientOverlay] Overlay %d now visible with %d layers spread horizontally" % [overlay_index, food_layer_sprites.size()])
