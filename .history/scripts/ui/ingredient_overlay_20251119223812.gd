...
func set_ingredient(ingredient_name: String, ingredient_textures: Array):
	...
	if ingredient_textures.size() == 1:
		# Single ingredient - center it
		var layer_sprite = Sprite2D.new()
		layer_sprite.texture = ingredient_textures[0]
		layer_sprite.position = Vector2.ZERO
		add_child(layer_sprite)
		food_layer_sprites.append(layer_sprite)
		print("[IngredientOverlay]   Added single layer centered")
	else:
		# Multiple ingredients - randomize order and position on opposite sides
		# First ingredient goes left, second goes right (or vice versa)
		var shuffled_textures = ingredient_textures.duplicate()
		shuffled_textures.shuffle()
		
		# Randomly choose which side gets first ingredient
		var first_goes_left = randf() > 0.5
		
		for i in range(shuffled_textures.size()):
			var layer_texture = shuffled_textures[i]
			if layer_texture:
				var layer_sprite = Sprite2D.new()
				layer_sprite.texture = layer_texture
				
				# Random distance from center (20-32 pixels)
				var distance = randf_range(20.0, 32.0)
				
				# Alternate sides: if first goes left, second goes right, etc.
				var goes_left = first_goes_left if i % 2 == 0 else not first_goes_left
				var direction = -1.0 if goes_left else 1.0
				
				layer_sprite.position = Vector2(distance * direction, 0.0)
				
				add_child(layer_sprite)
				food_layer_sprites.append(layer_sprite)
				print("[IngredientOverlay]   Added layer %d at x=%f (side: %s)" % [i, layer_sprite.position.x, "left" if goes_left else "right"])
			else:
