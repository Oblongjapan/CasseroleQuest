...
func _populate_shop_grid() -> void:
	...
	for item in items:
		var item_card = await _create_shop_item_card(item)
...
func _create_shop_item_card(item: Dictionary) -> Control:
	...
	if item.type == ShopManager.ShopItemType.NEW_INGREDIENT and item.has("ingredient"):
		return await _create_ingredient_purchase_card(item)
...
func _create_ingredient_purchase_card(item: Dictionary) -> Control:
	...
	var pending_upgrade = card_instance.get_meta("pending_upgrade", "") as String
	card_instance.setup(pending_ingredient, pending_upgrade)
