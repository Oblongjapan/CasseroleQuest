...
func _update_drain_preview() -> void:
	...
	# Water: SUM, Resistance: MIN (weakest), Volatility: MAX (most volatile)
	var total_water = ing1.water_content + ing2.water_content
	var combined_heat_resist = min(ing1.heat_resistance, ing2.heat_resistance)
	var combined_volatility = max(ing1.volatility, ing2.volatility)
