...
const SPRING_POSITION := Vector2(960, 400)  # Center of screen, where hand would be
const RECIPE_BOX_POSITION := Vector2(1700, 400)  # Right side where recipe box is
...
func show_reveal(recipe_ingredient: IngredientModel) -> void:
	...
	var display_name = recipe_ingredient.get_meta("display_name", recipe_ingredient.name)
