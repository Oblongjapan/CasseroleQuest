...
func _show_next_discovery():
	...
	if card.has_method("setup_from_ingredients"):
		...
		# Wait for the card animation to complete (spring + jiggle)
		if card.has_signal("reveal_completed"):
			await card.reveal_completed
		else:
			# Fallback: wait for typical animation time (spring 0.5s + jiggle 0.5s)
			await get_tree().create_timer(1.5).timeout
		
		# Auto-advance to next after a short pause
