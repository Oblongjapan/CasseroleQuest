...
func _update_placeholder_display(_tier: int):
	stats_label.hide()
	lock_icon.hide()
...
func _show_recipe_card():
	...
	# Try multiple paths since we might be in main menu or during gameplay
	var recipe_card_reveal = null
	
	# First try getting from root
	var root = get_tree().root
	if root:
		# Try Main/UI/RecipeCardReveal
		var main_node = root.get_node_or_null("Main")
		if main_node:
			recipe_card_reveal = main_node.get_node_or_null("UI/RecipeCardReveal")
		
		# If not found, try searching the entire tree
		if not recipe_card_reveal:
			for child in root.get_children():
				recipe_card_reveal = child.find_child("RecipeCardReveal", true, false)
				if recipe_card_reveal:
					break
	
	if not recipe_card_reveal:
		print("[RecipeBookEntry] ERROR: RecipeCardReveal not found in scene tree")
		return
	...
	if recipe_card_reveal.has_method("show_reveal"):
		...
		print("[RecipeBookEntry] ERROR: show_reveal method not found")
