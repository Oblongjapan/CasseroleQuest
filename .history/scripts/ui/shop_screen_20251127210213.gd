...
func _show_ingredient_added_feedback(ingredient: IngredientModel, start_position: Vector2) -> void:
	...
	# Position the card at the item's position (already in screen coordinates)
	feedback_card.global_position = start_position - (feedback_card.size / 2)
	feedback_card.z_index = 1001  # Above everything else
	feedback_card.scale = Vector2(0.8, 0.8)  # Start slightly smaller
	
	# Fridge position (center of the fridge background sprite)
	var fridge_global_position = Vector2(641, 360)
	...
	# Slide to fridge
	tween.tween_property(feedback_card, "global_position", fridge_global_position - (feedback_card.size / 2), 0.8)
