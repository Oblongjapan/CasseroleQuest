...
func _ready():
	...
	print("[IngredientCard] status_label: %s" % status_label)
	...
	if card_panel:
		...
		if theme_style:
			# If it's already a StyleBoxFlat, duplicate it so each card has its own copy
			if theme_style is StyleBoxFlat:
				...
				# If the theme uses a StyleBoxTexture or other StyleBox, convert to a flat style
				# so we can safely modify border_color/bg_color. This avoids assigning
				# a StyleBoxTexture to a StyleBoxFlat variable.
				card_style = StyleBoxFlat.new()
				# Reasonable defaults; these can be overridden via theme or editor later
				...
				card_panel.add_theme_stylebox_override("panel", card_style)
				print("[IngredientCard] Converted non-Flat StyleBox to StyleBoxFlat fallback")
			print("[IngredientCard] Style duplicated and assigned")
	else:
