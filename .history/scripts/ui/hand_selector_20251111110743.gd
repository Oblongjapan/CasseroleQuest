...
@onready var hand_container: HBoxContainer = get_node_or_null("CenterContainer/VBoxContainer/HandContainer") as HBoxContainer
@onready var title_label: Label = get_node_or_null("CenterContainer/VBoxContainer/TopPanel/VBoxContainer/TitleLabel") as Label
@onready var info_label: Label = get_node_or_null("CenterContainer/VBoxContainer/TopPanel/VBoxContainer/InfoLabel") as Label
@onready var drain_preview_label: Label = get_node_or_null("CenterContainer/VBoxContainer/TopPanel/VBoxContainer/DrainPreviewLabel") as Label
@onready var start_button: Button = get_node_or_null("CenterContainer/VBoxContainer/StartButton") as Button
...
func _ready():
	...
	print("[HandSelector] start_button: ", start_button)
	
	# Connect start button
	if start_button:
		start_button.pressed.connect(_on_start_button_pressed)
		start_button.hide()  # Hidden until 1-2 ingredients selected
...
func _populate_hand() -> void:
	...
	for i in range(hand_ingredients.size()):
		...
		if hand_container:
			...
			print("[HandSelector] Added card %d: %s (size: %v)" % [i, ingredient.name, card.size])
...
func _animate_cards_draw() -> void:
	...
	for i in range(card_buttons.size()):
		...
		tween.finished.connect(func(): 
			card.set_meta("original_global_position", card.global_position)
			print("[HandSelector] Card %d animation finished - final global_pos: %v" % [i, card.global_position])
		)
...
func _on_card_hover_end(card: IngredientCard) -> void:
...
func _on_card_input(event: InputEvent, card: IngredientCard) -> void:
...
func _on_start_button_pressed() -> void:
	print("[HandSelector] Start button pressed!")
	confirm_selection()
