...
# Child sprite for the food item
...
func _ready():
	...
	# Create the food sprite as a child
	food_sprite = Sprite2D.new()
	add_child(food_sprite)
	food_sprite.position = Vector2.ZERO  # Centered on the plate
...
func set_ingredient(ingredient_name: String, ingredient_texture: Texture2D):
	...
	print("[IngredientOverlay]   ingredient_texture: %s" % ingredient_texture)
	print("[IngredientOverlay]   plate_texture: %s" % plate_texture)
	print("[IngredientOverlay]   food_sprite: %s" % food_sprite)
	
	current_ingredient_name = ingredient_name
	
	# Set the plate as the base texture
	texture = plate_texture
	print("[IngredientOverlay]   Set base texture to plate: %s" % (texture != null))
	
	# Set the food item on top
	if food_sprite:
		if ingredient_texture:
			food_sprite.texture = ingredient_texture
			food_sprite.show()
			print("[IngredientOverlay]   Food sprite texture set and shown")
		else:
			print("[IngredientOverlay]   ERROR: ingredient_texture is null!")
	else:
		print("[IngredientOverlay]   ERROR: food_sprite is null!")
	
	show()
	print("[IngredientOverlay] Overlay %d now visible" % overlay_index)
