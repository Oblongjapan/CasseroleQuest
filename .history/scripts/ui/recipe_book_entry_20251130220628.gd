...
func update_display():
	...
	if is_discovered:
		...
		entry_label.text = "%s (T%d, %d ing) - %s" % [display_name, tier, ingredient_list.size(), ingredients_text]
		entry_label.modulate = Color(0.85, 0.75, 0.95, 1.0)
	else:
		# Undiscovered - show as locked with tier info only
		entry_label.text = "🔒 ??? (T%d, %d ing)" % [tier, ingredient_list.size()]
		entry_label.modulate = Color(0.35, 0.35, 0.35, 0.8)
