...
func _populate_hand() -> void:
	...
	# Get container size for centering (use a reliable fallback if size isn't set yet)
	var container_width: float = 800.0  # Default fallback
	if hand_container and hand_container.size.x > 10:  # Check for reasonable size
		container_width = hand_container.size.x
		print("[HandSelector] Using hand_container width: %.1f" % container_width)
	else:
		print("[HandSelector] Using fallback width: %.1f" % container_width)
