...
func _on_free_sample_taken(item: Dictionary, slot: ShopSlot):
	# Check if can take this sample
	var can_take = false
	
	if not slot.is_sample_seen:
		# Unseen sample - can take if under 2 unseen limit
		if samples_taken < max_unseen_samples:
			can_take = true
	else:
		# Seen sample - can only take 1 seen sample total
		if samples_taken == 0:
			can_take = true
			# Taking a seen sample maxes out the samples
			samples_taken = max_unseen_samples  # Set to max to prevent more
		else:
			_show_message("You can only take 2 unseen samples OR 1 seen sample!")
			return
	...
	if can_take:
		...
		if fridge_manager and item.has("ingredient"):
			...
			samples_taken += 1
			print("[VisualShopGrid] Free sample taken: %s (%d/%d)" % [item.get("name", ""), samples_taken, max_unseen_samples])
			_show_message("Added %s to deck!" % item.get("name", ""))
	else:
		_show_message("You've already taken the maximum free samples!")
