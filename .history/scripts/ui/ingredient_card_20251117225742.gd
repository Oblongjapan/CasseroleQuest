...
func setup(ing: IngredientModel, upgrade_desc: String = "") -> void:
	...
	if food_sprite:
		...
		if "+" in ingredient.name:
			...
			# Single ingredient - show it normally
...
func _setup_layered_food_display(recipe_name: String) -> void:
	...
	# Load plate texture if not already loaded
	...
	# Create a sprite for each ingredient and layer them on the plate
	for i in range(min(ingredient_names.size(), 2)):  # Limit to 2 for now
		...
		if texture:
			...
			layer_sprite.position = Vector2.ZERO  # Centered on plate
			...
			print("[IngredientCard] Added layer %d: %s" % [i, ing_name])
		else:
...
func _setup_sprite_hover_detection() -> void:
	...
	# Create a Control node to detect mouse hover over sprite area
	sprite_area = Control.new()
	sprite_area.mouse_filter = Control.MOUSE_FILTER_PASS
	food_sprite.add_child(sprite_area)
	
	# Position and size the area to match the sprite
	# Note: Sprite2D uses centered positioning by default
	if food_sprite.texture:
		var tex_size = food_sprite.texture.get_size()
		sprite_area.custom_minimum_size = tex_size
		sprite_area.size = tex_size
		sprite_area.position = -tex_size / 2  # Center it
	
	sprite_area.mouse_entered.connect(_on_sprite_mouse_entered)
	sprite_area.mouse_exited.connect(_on_sprite_mouse_exited)
