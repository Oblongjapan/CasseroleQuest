...
## When hiding, make sure to discard any drawn cards that weren't used
func _notification(what: int) -> void:
	if what == NOTIFICATION_VISIBILITY_CHANGED and not visible:
		# If we drew cards but never started cooking, return them to discard
		if drawn_ingredients.size() > 0 and fridge_manager:
			print("[IngredientSelector] Discarding %d unused cards on hide" % drawn_ingredients.size())
			fridge_manager.discard_cards(drawn_ingredients)
			drawn_ingredients.clear()
...
func show_selector(fridge: FridgeManager, currency: CurrencyManager, round_number: int) -> void:
	...
	# Update labels
	# Update labels (guard in case nodes are missing)
	if title_label:
		title_label.text = "Round %d" % current_round
	else:
		print("[IngredientSelector] WARNING: title_label not found")

	if round_label:
		round_label.text = "Next 2 Ingredients from Deck"
	else:
		print("[IngredientSelector] WARNING: round_label not found in scene; skipping")
...
func _update_drain_preview() -> void:
	...
	var avg_spice = (ing1.volatility + ing2.volatility) / 2.0
	var avg_heat = (ing1.heat_resistance + ing2.heat_resistance) / 2.0
	var estimated_drain = 5.0 + (avg_spice * 0.3) - (avg_heat * 0.25)
	estimated_drain = max(0.5, estimated_drain)
	
	drain_preview_label.text = "Estimated Drain: ~%.1f/sec" % [
		estimated_drain
	]
...
func _on_start_pressed() -> void:
	...
	print("[IngredientSelector] Discarding %d cards on start" % drawn_ingredients.size())
	fridge_manager.discard_cards(drawn_ingredients)
	
	# Emit round started signal
	EventBus.round_started.emit(drawn_ingredients[0], drawn_ingredients[1])
	
	# Clear the array so _notification doesn't double-discard
	drawn_ingredients.clear()
