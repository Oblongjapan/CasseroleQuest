...
func _update_drain_preview() -> void:
	...
	var total_volatility = ing1.volatility + ing2.volatility
	
	# Calculate drain rate (volatility multiplier is 0.20 for combined ingredients)
	var base_drain = 5.0
	var drain = base_drain + (total_volatility * 0.20) - (total_heat_resist * 0.035) - (total_density * 0.030)
	var drain_rate = maxf(drain, 1.0)
	...
	drain_preview_label.text = "Moisture Drain Rate: %.2f/sec%s" % [drain_rate, synergy_text]
