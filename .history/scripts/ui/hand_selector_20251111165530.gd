...
func show_hand_selection(fridge: FridgeManager) -> void:
	...
	# Draw cards to fill hand up to 3 (keep existing cards from previous round)
	var cards_to_draw = 3 - hand_ingredients.size()
	if cards_to_draw > 0:
		var new_cards = fridge_manager.draw_cards(cards_to_draw)
		hand_ingredients.append_array(new_cards)
		print("[HandSelector] Drew %d new cards, now have %d total in hand" % [new_cards.size(), hand_ingredients.size()])
	else:
		print("[HandSelector] Hand already full with %d cards" % hand_ingredients.size())
...
func confirm_selection() -> void:
	...
	# Only discard the selected cards (used in cooking)
	# Unselected cards stay in hand for next round
	print("[HandSelector] Discarding %d selected cards to fridge manager..." % selected_ingredients.size())
