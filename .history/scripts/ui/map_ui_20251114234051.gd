...
func _ready():
	# Try to resolve the map container here; if it's not present yet we'll resolve it later
	map_container = get_node_or_null("VBoxContainer/ScrollContainer/MapContainer")
	hide()

## Setup and display the map
func show_map(p_map_manager: MapManager) -> void:
	map_manager = p_map_manager
	# Ensure the container reference is valid before building UI
	if not map_container:
		map_container = get_node_or_null("VBoxContainer/ScrollContainer/MapContainer")
		if not map_container:
			print("[MapUI] ERROR: MapContainer node not found in scene. Aborting build.")
			show()
			return
	_build_map_ui()
	show()
...
func _build_map_ui() -> void:
	# Clear existing UI
	if not map_container:
		print("[MapUI] ERROR: map_container is null in _build_map_ui()")
		return
