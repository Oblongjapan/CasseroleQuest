...
@onready var make_organic_button: Button = $MakeOrganicButton
...
## Handle Make Organic button press
func _on_make_organic_pressed() -> void:
	print("[ShopScreen] Make Organic button pressed")
	if card_selector:
		card_selector.open_for_organic_upgrade(fridge_manager)
		card_selector.card_selected.disconnect(_on_card_selected_for_upgrade)
		card_selector.card_selected.connect(_on_card_selected_for_organic)
	else:
		print("[ShopScreen] Error: No card selector")

## Handle when card is selected for organic upgrade
func _on_card_selected_for_organic(ingredient_name: String) -> void:
	print("[ShopScreen] Card selected for organic: %s" % ingredient_name)
	shop_manager.make_ingredient_organic(ingredient_name)
	
	# Reconnect normal upgrade signal
	card_selector.card_selected.disconnect(_on_card_selected_for_organic)
	card_selector.card_selected.connect(_on_card_selected_for_upgrade)
