...
func show_shop(shop_mgr: ShopManager, currency_mgr: CurrencyManager, inv_mgr: InventoryManager, round_number: int) -> void:
...
func _on_upgrade_needs_target(upgrade_data: Dictionary) -> void:
	...
	card_selector.open_with_upgrade(upgrade_data, inventory_manager)
