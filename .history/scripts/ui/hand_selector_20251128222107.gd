...
var selected_ingredients: Array[IngredientModel] = []  # 1-2 selected via drag (3 if tier 2 unlocked)
var fridge_manager: FridgeManager
var game_state_manager: GameStateManager  # For water cup check
var progression_manager: ProgressionManager  # For tier unlock checks
...
signal selection_ready(can_start: bool)  # Emitted when 1-3 ingredients are selected (based on tier unlocks)
...
## If after_shop is true, clears hand and draws cards with persistent cards included
## If after_shop is false (between rounds), keeps existing hand and draws to fill to 5
func show_hand_selection(fridge: FridgeManager, game_state_mgr: GameStateManager = null, prog_mgr: ProgressionManager = null, after_shop: bool = false) -> void:
	...
	if after_shop:
		# After shop: keep remaining cards in hand, add shop samples, draw to fill to 5
		print("[HandSelector] After shop - current hand has %d cards" % hand_ingredients.size())
		
		# Get persistent cards (shop samples)
		var persistent_cards = fridge_manager.claim_persistent_hand()
		hand_ingredients.append_array(persistent_cards)
		print("[HandSelector] Added %d shop samples to hand, total: %d" % [persistent_cards.size(), hand_ingredients.size()])
		
		# Draw additional cards to reach 5 total
		var cards_to_draw = max(0, 5 - hand_ingredients.size())
		if cards_to_draw > 0:
			var new_cards = fridge_manager.draw_cards(cards_to_draw)
			hand_ingredients.append_array(new_cards)
			print("[HandSelector] Drew %d additional cards, hand size: %d" % [cards_to_draw, hand_ingredients.size()])
	else:
		# Between rounds OR game start
		# Game start: hand is empty, so draw to 5
		# Between rounds: hand has cards, draw 1 to add
		if hand_ingredients.is_empty():
			# Game start - draw 5 cards
			var new_cards = fridge_manager.draw_cards(5)
			hand_ingredients.append_array(new_cards)
			print("[HandSelector] Game start - drew 5 cards, hand size: %d" % hand_ingredients.size())
		else:
	...
	if info_label:
		var max_ingredients = _get_max_ingredients()
		info_label.text = "Drag ingredients onto the microwave to select (1-%d)" % max_ingredients
...
func _check_drop_on_microwave(card: IngredientCard) -> void:
	...
	var microwave = get_node_or_null("/root/Main/Background/Microwave")
	if not microwave:
...
## Get maximum number of ingredients that can be selected based on tier unlocks
func _get_max_ingredients() -> int:
	if progression_manager and progression_manager.tier_2_unlocked:
		return 3
	else:
		return 2
...
func _toggle_card_selection(ingredient: IngredientModel, card: IngredientCard) -> void:
	if ingredient in selected_ingredients:
		...
		# Check max ingredients based on tier
		var max_ingredients = _get_max_ingredients()
		
		# Select (if under limit)
		if selected_ingredients.size() < max_ingredients:
			# Find first available overlay slot (0, 1, or 2)
			...
			# At max limit - show feedback if trying to select 3rd ingredient without tier 2
			if max_ingredients == 2 and selected_ingredients.size() == 2:
				print("[HandSelector] Cannot select 3rd ingredient - Tier 2 not unlocked yet")
				# Show feedback message to player
				if info_label:
					var original_text = info_label.text
					info_label.text = "Unlock Tier 2 (6 unique recipes) to use 3 ingredients!"
					info_label.modulate = Color(1.0, 0.5, 0.5, 1.0)  # Red tint
					
					# Reset after 2 seconds
					await get_tree().create_timer(2.0).timeout
					if info_label:
						info_label.text = original_text
						info_label.modulate = Color(1.0, 1.0, 1.0, 1.0)  # Reset to white
			else:
...
## Find the first available overlay slot (0, 1, or 2)
...
func _update_selection_state() -> void:
	var max_ingredients = _get_max_ingredients()
	var can_start = selected_ingredients.size() >= 1 and selected_ingredients.size() <= max_ingredients
	print("[HandSelector] _update_selection_state: %d ingredients selected (max: %d), can_start: %s" % [selected_ingredients.size(), max_ingredients, can_start])
...
func confirm_selection() -> void:
	...
	var max_ingredients = _get_max_ingredients()
	if selected_ingredients.size() < 1 or selected_ingredients.size() > max_ingredients:
		print("[HandSelector] Cannot confirm: need 1-%d ingredients selected" % max_ingredients)
		return
	
	# Remove selected cards from hand_ingredients - unselected cards stay for next round
	print("[HandSelector] Removing %d selected cards from hand..." % selected_ingredients.size())
	for ingredient in selected_ingredients:
		hand_ingredients.erase(ingredient)
	
	print("[HandSelector] Remaining cards in hand: %d (will persist to next round)" % hand_ingredients.size())
