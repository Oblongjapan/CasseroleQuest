...
func set_item(p_item: ActiveItem, p_index: int) -> void:
	...
	# Only set text if this has a text property (Button, not TextureButton)
	if "text" in self:
		self.text = item.name
...
func _on_item_used(index: int) -> void:
	if index == item_index and item != null:
		is_ready = false
		disabled = true
...
func _on_cooldown_updated(index: int, remaining: float) -> void:
	if index == item_index and item != null:
		...
		if cooldown_remaining > 0:
			# Only update text if this has a text property (Button, not TextureButton)
			if "text" in self:
				self.text = "%s (%.1f)" % [item.name, cooldown_remaining]
			is_ready = false
			disabled = true
		else:
			# Only update text if this has a text property (Button, not TextureButton)
