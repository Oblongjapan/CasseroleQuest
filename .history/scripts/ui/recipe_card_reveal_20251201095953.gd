...
var is_discovery_reveal: bool = false  # Track if this is a discovery reveal (no hint label)

const CARD_SCENE_PATH = "res://scenes/ingredient_card.tscn"
const CARD_HOVER_SCALE := 1.2
const CARD_NORMAL_SCALE := Vector2(1.0, 1.0)
const SPRING_POSITION := Vector2(465, 480)  # Default position for in-game reveals
const RECIPE_BOX_POSITION := Vector2(919, 200)  # where recipe box is
const FLIP_DURATION := 0.3
const SLIDE_DURATION := .8

var spring_target_position: Vector2 = SPRING_POSITION  # Can be overridden for centered reveals
...
## target_position: Optional override for card position (e.g. center for discovery reveals)
func show_reveal(recipe_ingredient: IngredientModel, target_position: Vector2 = SPRING_POSITION) -> void:
	spring_target_position = target_position
	print("[RecipeCardReveal] Starting reveal for: %s at position %s" % [recipe_ingredient.name, spring_target_position])
	is_dismissed = false
	is_hovering = false
	...
	# Set pivot offset to center of card for rotation
	if revealed_card:
		revealed_card.pivot_offset = revealed_card.size / 2.0
	
	# Position card off-screen (below) for spring animation
	var start_position = spring_target_position + Vector2(0, 600)
	revealed_card.global_position = start_position
	...
	original_card_position = spring_target_position
	...
	# Show hint label only for normal in-game reveals (not discovery reveals)
	if hint_label:
		if is_discovery_reveal:
			hint_label.hide()
		else:
			hint_label.show()
	...
	# Add jiggle effect for new discovery
	await _animate_card_jiggle()
	
	print("[RecipeCardReveal] Card is now interactive - hover to scale, click to dismiss")

## Setup card from ingredient names (for discovery reveal)
func setup_from_ingredients(ingredient_names: Array, _display_name: String) -> void:
	print("[RecipeCardReveal] Setting up card from ingredients: %s" % ", ".join(ingredient_names))
	
	# Mark this as a discovery reveal (no hint label)
	is_discovery_reveal = true
	
	# Create a combined ingredient model
	var combined = RecipesData.combine_ingredients_from_names(ingredient_names)
	if combined:
		# Use centered position for discovery reveals (640, 360 for 1280x720 screen)
		show_reveal(combined, Vector2(640, 360))
	else:
		print("[RecipeCardReveal] ERROR: Failed to create combined ingredient")
...
func _animate_card_spring_in() -> void:
	...
	tween.tween_property(revealed_card, "global_position", spring_target_position, 0.5)
...
## Animate a fast jiggle/rock effect
func _animate_card_jiggle() -> void:
	if not revealed_card:
		return
	
	var tween = create_tween()
	var jiggle_angle = 15.0  # degrees
	var jiggle_duration = 0.08  # fast jiggle
	
	# Rock back and forth 3 times
	tween.tween_property(revealed_card, "rotation_degrees", jiggle_angle, jiggle_duration)
	tween.tween_property(revealed_card, "rotation_degrees", -jiggle_angle, jiggle_duration * 2)
	tween.tween_property(revealed_card, "rotation_degrees", jiggle_angle, jiggle_duration * 2)
	tween.tween_property(revealed_card, "rotation_degrees", -jiggle_angle, jiggle_duration * 2)
	tween.tween_property(revealed_card, "rotation_degrees", 0.0, jiggle_duration)
	
	await tween.finished
	print("[RecipeCardReveal] Jiggle animation complete")
