...
func _show_recipe_card():
	...
	# Find the recipe book in the tree and signal it to show the card
	var recipe_book = get_parent()
	while recipe_book and not recipe_book.has_method("show_recipe_card_popup"):
		recipe_book = recipe_book.get_parent()
	
	if recipe_book and recipe_book.has_method("show_recipe_card_popup"):
		recipe_book.show_recipe_card_popup(combined_ingredient)
	else:
		print("[RecipeBookEntry] ERROR: Recipe book with show_recipe_card_popup method not found")
