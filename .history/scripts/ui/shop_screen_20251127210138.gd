	...
	if currency_manager.get_currency() >= cost:
		if currency_manager.spend_currency(cost):
			if fridge_manager and item.has("ingredient"):
				...
				# Show animation feedback after adding to deck
				_show_ingredient_added_feedback(item.ingredient, container.get_global_rect().get_center())
...
func _on_hover_take_pressed():
	...
	if fridge_manager and current_hover_item.has("ingredient"):
		...
		# Show animation feedback after adding to deck
		_show_ingredient_added_feedback(current_hover_item.ingredient, current_hover_container.get_global_rect().get_center())
...
## Show ingredient added feedback animation - slides ingredient card to fridge and fades out
func _show_ingredient_added_feedback(ingredient: IngredientModel, start_position: Vector2) -> void:
	# Create an ingredient card for the animation
	var feedback_card = IngredientCardScene.instantiate()
	add_child(feedback_card)
	feedback_card.setup(ingredient)
	
	# Wait a frame for the card to be ready, then animate
	await get_tree().process_frame
	
	# Position the card at the item's position (convert to local coordinates)
	feedback_card.position = to_local(start_position) - (feedback_card.size / 2)
	feedback_card.z_index = 1001  # Above everything else
	feedback_card.scale = Vector2(0.8, 0.8)  # Start slightly smaller
	
	# Fridge position (center of the fridge background sprite in local coordinates)
	var fridge_position = to_local(Vector2(641, 360))
	
	# Create animation tween - slide to fridge and fade out
	...
	# Slide to fridge
	tween.tween_property(feedback_card, "position", fridge_position - (feedback_card.size / 2), 0.8)
	
	# Scale down as it approaches
	tween.tween_property(feedback_card, "scale", Vector2(0.3, 0.3), 0.8)
	
	# Fade out
	tween.tween_property(feedback_card, "modulate:a", 0.0, 0.8)
	
	# Clean up after animation completes
	tween.finished.connect(func(): 
		if is_instance_valid(feedback_card):
			feedback_card.queue_free()
		print("[ShopScreen] Ingredient added feedback animation complete")
	)
