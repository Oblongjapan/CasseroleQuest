...
@onready var confirm_button: Button = $CenterContainer/VBoxContainer/BottomPanel/CenterContainer2/ConfirmButton
...
func _on_confirm_pressed() -> void:
	...
	# Find the discarded ingredients (return to top of deck)
	for ingredient in hand_ingredients:
		if ingredient not in selected_ingredients:
			fridge_manager.deck.push_front(ingredient)
	
	# Discard the selected cards (they'll be used this round)
