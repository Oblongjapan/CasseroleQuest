...
func _update_drain_preview() -> void:
	...
	# Calculate drain rate (spice multiplier is 0.20 for combined ingredients)
	var base_drain = 5.0
	var drain = base_drain + (total_spice * 0.20) - (total_heat_resist * 0.035) - (total_density * 0.030)
