...
func _ready():
	...
	if hand_selector:
		...
		# selection_ready signal is now handled internally by hand_selector
	...
	# Connect ON button
	if on_button:
		on_button.pressed.connect(_on_start_button_pressed)
		on_button.visible = false  # Start hidden until ingredients are selected
		print("[Main] ON Button connected successfully")
...
func _on_selection_ready(can_start: bool) -> void:
	...
	if on_button:
		on_button.visible = can_start
		on_button.disabled = not can_start
		print("[Main] ON Button visibility set to: %s" % can_start)

## Handle ON button press (confirm hand selection)
func _on_start_button_pressed() -> void:
	print("[Main] ========================================")
	print("[Main] ON Button pressed!")
	print("[Main] ========================================")
	if hand_selector:
		print("[Main] Calling hand_selector.confirm_selection()")
		hand_selector.confirm_selection()
	else:
		print("[Main] ERROR: hand_selector is null!")
