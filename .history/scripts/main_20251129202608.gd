...
## Show upgrade highlight on card - changes the upgraded stat to green
func _show_upgrade_highlight(card: Control, upgrade_data: Dictionary) -> void:
	...
	# Find the specific stat label to color green
	...
	# Change the stat label color to green permanently
	target_label.add_theme_color_override("font_color", Color(0.2, 1.0, 0.2))  # Bright green
	
	# Hold for 2 seconds to show the card
	await get_tree().create_timer(2.0).timeout
