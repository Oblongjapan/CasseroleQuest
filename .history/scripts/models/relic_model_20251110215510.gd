...
enum EffectType {
	...
	BONUS_STAT,          # Adds a flat stat bonus to ingredients (e.g., +Heat)
...
func get_effect_description() -> String:
	match effect_type:
		...
		EffectType.BONUS_STAT:
			# Generic stat bonus description - shop/relic data should clarify which stat
			return "+%d to ingredient stat" % int(effect_value)
		EffectType.SLOWER_DIFFICULTY:
			return "-%d%% difficulty scaling" % int(effect_value * 100)
		_:
			return "Unknown effect"

## Get symbol/emoji for the relic type
func get_symbol() -> String:
	match effect_type:
		EffectType.REDUCE_DRAIN:
			return "🛡️"  # Shield for drain reduction
		EffectType.REDUCE_COOLDOWN:
			return "⚡"  # Lightning for speed
		EffectType.BONUS_MOISTURE:
			return "💧"  # Water droplet for moisture
		EffectType.BONUS_STAT:
			return "🔥"  # Fire for heat/stat bonus (used as generic stat icon)
		EffectType.SLOWER_DIFFICULTY:
			return "📚"  # Book for learning/training
		_:
			return "❓"
