...
func _check_and_create_recipe() -> void:
	...
	if combined_ingredient:
		...
		# Add combo to deck and discard the original ingredients
		fridge_manager.add_ingredient_to_deck(combined_ingredient)
		
		# The original ingredients are consumed (not returned to discard)
		...
		print("[Main] Original %d ingredients consumed (not returned to deck)" % current_round_ingredients.size())
	else:
