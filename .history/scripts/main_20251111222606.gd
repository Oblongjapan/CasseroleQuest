...
func _on_round_started(ingredient_1: IngredientModel, ingredient_2) -> void:
	...
	# Check if malfunction should trigger this round
	...
	if is_malfunction_round and current_malfunction:
		...
		match current_malfunction.type:
			MalfunctionModel.MalfunctionType.OVERHEAT:
				...
				print("[Main] OVERHEAT: Drain rate 1.5x")
			
			MalfunctionModel.MalfunctionType.BULB_OUT:
				# 0.5x drain rate
				moisture_manager.set_malfunction_multiplier(0.5)
				timer_manager.start_timer(15.0)
				print("[Main] BULB OUT: Drain rate 0.5x")
			
			MalfunctionModel.MalfunctionType.MACROWAVE:
				# 1 second per card in hand
				var hand_size = fridge_manager.get_hand_size()
				timer_manager.start_macrowave_timer(hand_size)
				moisture_manager.reset_malfunction_multiplier()
				print("[Main] MACROWAVE: Timer set to %d seconds" % hand_size)
	else:
		# Normal timer
		timer_manager.start_timer(15.0)
		moisture_manager.reset_malfunction_multiplier()
...
func _on_round_completed(success: bool, final_moisture: float) -> void:
	...
	if is_malfunction_round and current_malfunction:
		...
		# Only show rewards if successful
		if success:
			print("[Main] Malfunction succeeded! Showing reward for: %s" % current_malfunction.name)
...
func _on_shop_opened() -> void:
	...
	# Show shop screen
	if shop_screen:
		shop_screen.show_shop(shop_manager, currency_manager, fridge_manager, current_round_number - 1)
...
func _show_malfunction_reward(malfunction: MalfunctionModel) -> void:
	...
	match malfunction.type:
		MalfunctionModel.MalfunctionType.OVERHEAT:
			...
			if malfunction_relic_reward:
				malfunction_relic_reward.show_relic_selection()
