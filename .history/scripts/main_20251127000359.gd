...
func _show_hover_card(ingredient: IngredientModel, btn: TextureButton) -> void:
	...
	# Position the card centered on the button's texture/sprite (not the button itself)
	# TextureButtons display the texture centered, so we center on the button position
	var btn_global_pos = btn.global_position
	var btn_center = btn_global_pos + (btn.size / 2)
	var card_size = hover_card.size
	
	# Center the hover card horizontally on the button's center, position above
	hover_card.global_position = Vector2(
		btn_center.x - (card_size.x / 2),
		btn_global_pos.y - card_size.y - 10  # 10px gap above button
	)
