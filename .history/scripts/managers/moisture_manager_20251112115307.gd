...
func update_moisture(delta: float) -> void:
	# Start with base drain and apply malfunction multiplier
	var total_drain = base_drain_rate * malfunction_drain_multiplier
	...
	if Engine.get_process_frames() % 60 == 0:
		print("[MoistureManager] Update: %.1f → %.1f (drain: %.2f/s, delta: %.4f)" % [old_moisture, current_moisture, total_drain, delta])
