...
func refresh_shop(round_number: int) -> void:
	...
	# Generate ingredient upgrades for owned ingredients
	_add_ingredient_upgrades()
	
	# Add relics (cost scales with round)
	_add_relics(round_number)
	
	# Add new ingredient offer
	_add_new_ingredient()
	
	# Add active item upgrades (if round >= 2)
	if round_number >= 2:
		_add_active_items()
