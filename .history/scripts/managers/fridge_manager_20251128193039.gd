...
func set_progression_manager(manager: ProgressionManager) -> void:
	...
	print("[FridgeManager] Progression manager linked")
...
func discard_cards(cards: Array[IngredientModel]) -> void:
	print("[FridgeManager] discard_cards() called with %d cards - before: deck=%d, discard=%d, hand=%d" % [cards.size(), deck.size(), discard_pile.size(), current_hand.size()])
	for card in cards:
	...
	print("[FridgeManager] Discarded %d cards, discard pile now has %d (deck: %d, hand: %d)" % [cards.size(), discard_pile.size(), deck.size(), current_hand.size()])
	_emit_deck_changed()
...
func consume_cards(cards: Array[IngredientModel]) -> void:
	print("[FridgeManager] consume_cards() called with %d cards" % cards.size())
	for card in cards:
		var hand_index = current_hand.find(card)
		if hand_index != -1:
			current_hand.remove_at(hand_index)
			print("[FridgeManager]   Consumed from hand: %s" % card.name)
	print("[FridgeManager] Consumed %d cards, hand now has %d" % [cards.size(), current_hand.size()])
	_emit_deck_changed()
...
func _emit_deck_changed() -> void:
	...
	print("[FridgeManager] Deck changed: deck=%d, discard=%d, hand=%d, total=%d" % [deck.size(), discard_pile.size(), cards_in_hand, total])
...
func add_to_persistent_hand(ingredient: IngredientModel) -> void:
	...
	print("[FridgeManager] Added %s to persistent hand (size: %d)" % [ingredient.name, persistent_hand.size()])

## Save last card from current hand to persistent hand (end of 3-round chunk)
func save_last_card_to_persistent() -> void:
	if current_hand.is_empty():
		print("[FridgeManager] No cards in current hand to save as persistent")
		return
	
	# Save the last card from hand
	var last_card = current_hand[current_hand.size() - 1]
	persistent_hand.append(last_card)
	current_hand.remove_at(current_hand.size() - 1)
	print("[FridgeManager] Saved last card to persistent: %s (persistent size: %d)" % [last_card.name, persistent_hand.size()])
...
func claim_persistent_hand() -> Array[IngredientModel]:
	...
	print("[FridgeManager] Claimed %d persistent cards" % cards.size())
	return cards

## Clear persistent hand (used when starting new 3-round chunk after shop)
func clear_persistent_hand() -> void:
	persistent_hand.clear()
	print("[FridgeManager] Cleared persistent hand")
