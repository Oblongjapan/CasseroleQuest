...
var round_modifier_manager: RoundModifierManager  # For shop discounts
...
func setup(fridge: FridgeManager, currency: CurrencyManager, inventory: InventoryManager, modifier_manager: RoundModifierManager = null) -> void:
	fridge_manager = fridge
	currency_manager = currency
	inventory_manager = inventory
	round_modifier_manager = modifier_manager
...
## Get the actual cost after applying round modifier discount
func get_modified_cost(base_cost: int) -> int:
	if round_modifier_manager:
		var discount = round_modifier_manager.get_shop_discount()
...
func _add_active_items() -> void:
	# Basic starter items at low cost
	var basic_items = [
		{
	...
	# Randomly add 1-2 basic items to shop
	basic_items.shuffle()
	for i in range(min(2, basic_items.size())):
		current_shop_items.append(basic_items[i])
...
func purchase_item(item: Dictionary) -> bool:
	...
	match item.type:
		...
		ShopItemType.ACTIVE_ITEM:
			# Create the active item and add to inventory
			var item_data = item.item_data
			var item_type = item_data.get("type", ActiveItem.Type.COVER)  # Default to COVER if not specified
			...
			if inventory_manager.can_add_active_item():
				inventory_manager.add_active_item(new_item)
				print("[ShopManager] Active item purchased: %s" % new_item.name)
			else:
				print("[ShopManager] Cannot add active item - inventory full")
				return false
