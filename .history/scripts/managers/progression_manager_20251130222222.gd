		...
		for tier in range(current_tier + 1, new_tier + 1):
			...
			match tier:
				1:
					...
					print("[ProgressionManager] 🎉 TIER 1 UNLOCKED: 2-Ingredient Recipes!")
					print("[ProgressionManager]    You can now combine any 2 ingredients!")
				2:
					tier_2_unlocked = true
					tier_unlocked.emit(2)
					print("[ProgressionManager] 🎉 TIER 2 UNLOCKED: 3+ Ingredient Recipes!")
					print("[ProgressionManager]    You can now combine 3 or more ingredients!")
				3:
					tier_3_unlocked = true
					tier_unlocked.emit(3)
					print("[ProgressionManager] 🎉 TIER 3 UNLOCKED: Advanced Recipes!")
				4:
					tier_4_unlocked = true
					tier_unlocked.emit(4)
					print("[ProgressionManager] 🎉 TIER 4 UNLOCKED: Legendary Recipes!")
				5:
					tier_unlocked.emit(5)
					print("[ProgressionManager] 🎉 TIER 5 UNLOCKED: Epic Recipes!")
				6:
					tier_unlocked.emit(6)
					print("[ProgressionManager] 🎉 TIER 6 UNLOCKED: Mythic Recipes!")
				7:
					tier_unlocked.emit(7)
					print("[ProgressionManager] 🎉 TIER 7 UNLOCKED: Godlike Recipes!")
				8:
					tier_unlocked.emit(8)
					print("[ProgressionManager] 🎉 TIER 8 UNLOCKED: ULTIMATE RECIPES - ALL 13 INGREDIENTS!")
...
func _check_milestones() -> void:
	...
	if unique_recipes_created >= 5 and microwave_power < 1.3:
		microwave_power = 1.3
	...
	elif unique_recipes_created >= 10 and microwave_power < 1.6:
		microwave_power = 1.6
	...
	elif unique_recipes_created >= 15 and microwave_power < 2.0:
		microwave_power = 2.0
		power_level_increased.emit(microwave_power)
...
## Get total unique recipes created count
