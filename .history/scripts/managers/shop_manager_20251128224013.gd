...
var purchased_items: Array[Dictionary] = []  # Track purchased items in current shop instance
...
func refresh_shop(round_number: int) -> void:
	...
	purchased_items.clear()  # Reset purchases for new shop instance
...
## Add relic options
func _add_relics(_round_number: int) -> void:
	var relics = [
		{
			"name": "Plastic Wrap",
...
func _add_new_ingredient() -> void:
	...
	if not all_ingredients.is_empty():
		var ingredient = all_ingredients[0].duplicate()
		
		# Apply 1 random upgrade to ingredient
		var upgrade = _get_random_stat_upgrade()
		_apply_stat_modification_to_ingredient(ingredient, upgrade)
		
		# Track which stat was upgraded
		var upgraded_stat = ""
		...
		# Store upgrade info in ingredient metadata
		ingredient.set_meta("upgraded_stats", [upgraded_stat])
		ingredient.set_meta("is_upgraded", true)
		
		# Add "Organic" prefix to upgraded ingredient name
		if not ingredient.name.begins_with("Organic "):
			ingredient.name = "Organic " + ingredient.name
		
		# Build upgrade description
		var upgrade_desc = ""
		for key in upgrade.keys():
			if key != "cost":
				var amount = upgrade[key]
				if key == "volatility" and amount < 0:
					upgrade_desc = "%d %s" % [amount, _stat_display_name(key)]
				else:
					upgrade_desc = "+%d %s" % [amount, _stat_display_name(key)]
		
		current_shop_items.append({
			"type": ShopItemType.NEW_INGREDIENT,
			"name": ingredient.name,  # Now displays as "Organic [Ingredient]"
...
func purchase_item(item: Dictionary) -> bool:
	...
	if item.get("is_purchased", false):
		print("[ShopManager] Item already purchased: %s" % item.name)
		return false
...
## Make an ingredient organic by applying random stat upgrade and adding organic tag
func make_ingredient_organic(ingredient_name: String) -> void:
	print("[ShopManager] Making ingredient organic: %s" % ingredient_name)
	
	# Generate random stat upgrade with balanced values
	var stats = ["water", "heat_resistance", "volatility"]
	var stat = stats[randi() % stats.size()]
	var amount = 0
	
	# Determine amount based on stat type
	match stat:
		"water":
			amount = randi_range(3, 8)  # +3 to +8 water
		"heat_resistance":
			amount = randi_range(3, 8)  # +3 to +8 heat resistance
		"volatility":
			amount = -randi_range(2, 5)  # -2 to -5 volatility (reduction is good)
	
	var modifications = {stat: amount}
	
	# Apply upgrade via fridge manager
	fridge_manager.upgrade_ingredient_stats(ingredient_name, modifications)
	
	# Add organic tag by adding "Organic " prefix if not already organic
	fridge_manager.make_ingredient_organic(ingredient_name)
	
	print("[ShopManager] Made %s organic with %s: %+d" % [ingredient_name, _stat_display_name(stat), amount])
...
func refresh_super_shop() -> void:
	...
	for i in range(2):
		if all_ingredients.size() > 0:
			...
			# Track which stats were upgraded
			var upgraded_stats: Array[String] = []
			for key in upgrade1.keys():
				if key != "cost":
					upgraded_stats.append(key)
			for key in upgrade2.keys():
				if key != "cost" and key not in upgraded_stats:
					upgraded_stats.append(key)
			
			base_ingredient.set_meta("upgraded_stats", upgraded_stats)
			base_ingredient.set_meta("is_upgraded", true)
			
			# Add "Organic" prefix to upgraded ingredient name
			if not base_ingredient.name.begins_with("Organic "):
				base_ingredient.name = "Organic " + base_ingredient.name
			
			# Create shop item for this upgraded ingredient
			var upgrade_desc = ""
			for key in upgrade1.keys():
				if key != "cost":
					var amount = upgrade1[key]
					if key == "volatility" and amount < 0:
						upgrade_desc += "%d %s, " % [amount, _stat_display_name(key)]
					else:
						upgrade_desc += "+%d %s, " % [amount, _stat_display_name(key)]
			...
			current_shop_items.append({
				"type": ShopItemType.NEW_INGREDIENT,
				"name": base_ingredient.name,  # Now displays as "Organic [Ingredient]"
				"description": "Upgraded: %s" % upgrade_desc,
				"cost": 100,
...
## Get random ingredients for the shop (free samples)
func get_shop_samples(count: int = 3) -> Array[IngredientModel]:
	# Get a pool of random ingredients
	# We want to favor base ingredients but include some others
	# For now, we'll just use the random pool from IngredientsData
	return IngredientsData.get_random_ingredient_pool(count)
