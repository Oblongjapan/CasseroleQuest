...
var microwave_drain: float = 5.0  # The microwave's base drain component
var ingredient_drain: float = 0.0  # The ingredient-specific drain component
var active_drain_modifiers: Array[Dictionary] = []  # {amount: float, duration: float}
var malfunction_drain_multiplier: float = 1.0  # Multiplier from active malfunction
...
func setup(ingredient_1, ingredient_2 = null, difficulty = 1.0, inventory = null) -> void:
	...
	# - base_drain_rate = 5.0 + (worst_spice × 0.3) - (best_heat × 0.15)
	
	# Check if solo cooking (same ingredient used twice)
	var is_solo = (ingredient_2 == null or ingredient_2 == ingredient_1)
	
	var total_water = ingredient_1.water_content
	if ingredient_2 != null and ingredient_2 != ingredient_1:
	...
	# Updated formula with adjusted multipliers for better balance:
	# - Reduced heat resistance effect from 0.15 to 0.10
	# - Increased volatility effect from 0.3 to 0.4
	# Separate microwave base drain from ingredient-specific drain
	microwave_drain = 5.0
	ingredient_drain = (worst_spice * 0.4) - (best_heat * 0.10)
	
	var drain = microwave_drain + ingredient_drain
	drain = drain * difficulty  # Apply difficulty multiplier
	base_drain_rate = max(0.5, drain)  # Clamp to minimum of 0.5/sec
	
	# Also scale the components
	microwave_drain = microwave_drain * difficulty
	ingredient_drain = ingredient_drain * difficulty
	
	# Debug output
	print("=== Moisture Setup ===")
	print("Cooking Mode: %s" % ("SOLO" if is_solo else "DUO"))
	...
	print("Microwave drain: %.2f/sec | Ingredient drain: %.2f/sec" % [microwave_drain, ingredient_drain])
	...
	if inventory:
		...
		microwave_drain *= (1.0 - drain_reduction)
		ingredient_drain *= (1.0 - drain_reduction)
...
func update_moisture(delta: float) -> void:
	# Apply malfunction multiplier ONLY to microwave drain, not ingredient drain
	var microwave_component = microwave_drain * malfunction_drain_multiplier
	var total_drain = microwave_component + ingredient_drain
	...
	var old_moisture = current_moisture
	...
	if Engine.get_process_frames() % 60 == 0:
		print("[MoistureManager] Update: %.1f → %.1f (drain: %.2f/s [MW:%.2f ING:%.2f], delta: %.4f)" % [
			old_moisture, current_moisture, total_drain, microwave_component, ingredient_drain, delta])
...
## Set malfunction drain multiplier (1.5x for Overheat, 0.5x for Bulb Out)
func set_malfunction_multiplier(multiplier: float) -> void:
	malfunction_drain_multiplier = multiplier
	print("[MoistureManager] Malfunction drain multiplier set to: %.1fx" % multiplier)

## Reset malfunction multiplier to normal
func reset_malfunction_multiplier() -> void:
	malfunction_drain_multiplier = 1.0
	print("[MoistureManager] Malfunction drain multiplier reset to normal")
