...
## Manages the player's collection of ingredients and relics
...
## Calculate total heat resistance bonus from all BONUS_STAT relics (like Plastic Wrap)
func get_total_heat_bonus() -> int:
	var bonus = 0
	for relic in owned_relics:
		if relic.effect_type == RelicModel.EffectType.BONUS_STAT:
			bonus += int(relic.effect_value)
	return bonus
