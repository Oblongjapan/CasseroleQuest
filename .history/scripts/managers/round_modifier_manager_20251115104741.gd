...
const RoundModifiersData = preload("res://scripts/data/round_modifiers.gd")
...
## Get a random modifier without setting it as current (for map generation)
func get_random_modifier() -> RoundModifierModel:
	return RoundModifiersData.get_random_modifier()

## Manually set a specific modifier (for map node selection)
func set_modifier(modifier: RoundModifierModel) -> void:
	current_modifier = modifier
	
	# Update shop discount if applicable
	if current_modifier and current_modifier.modifier_type == RoundModifierModel.ModifierType.SHOP_DISCOUNT:
		shop_discount = current_modifier.value
	else:
		shop_discount = 0.0
	...
	modifier_changed.emit(current_modifier)
	if current_modifier:
		print("[RoundModifierManager] Modifier set: %s" % current_modifier.name)
	else:
		print("[RoundModifierManager] Modifier cleared (set to null)")
