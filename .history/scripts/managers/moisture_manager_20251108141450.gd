...
func setup(ingredient_1: IngredientModel, ingredient_2: IngredientModel, difficulty: float = 1.0, inventory: InventoryManager = null) -> void:
	...
	if ingredient_1 == ingredient_2:
		# Using same ingredient twice - just use its drain rate
		base_drain_rate = drain_1
	else:
		# Using two different ingredients - average them but add 10% complexity penalty
		base_drain_rate = ((drain_1 + drain_2) / 2.0) * 1.1
