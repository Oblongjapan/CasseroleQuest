...
func initialize_starting_deck() -> void:
	...
	# Get all ingredients and categorize them
	var all_ingredients = IngredientsData.get_all_ingredients()
	var balanced_ingredients: Array[IngredientModel] = []
	var good_ingredients: Array[IngredientModel] = []
	
	# Categorize ingredients by survivability (water - volatility/2)
	for ing in all_ingredients:
		var survivability = ing.water_content - (ing.volatility / 2.0)
		if survivability >= 45:  # Good ingredients (easier)
			good_ingredients.append(ing)
		else:
			balanced_ingredients.append(ing)
	
	# Ensure starting deck has a good balance:
	# - At least 5 good ingredients (higher water or lower volatility)
	# - Up to 5 balanced/challenging ingredients
	good_ingredients.shuffle()
	balanced_ingredients.shuffle()
	
	# Add 5-6 good ingredients for reliable starts
	var good_count = min(6, good_ingredients.size())
	for i in range(good_count):
		deck.append(good_ingredients[i].duplicate())
	
	# Fill remaining slots with balanced ingredients
	var remaining_slots = 10 - good_count
	for i in range(remaining_slots):
		if i < balanced_ingredients.size():
			deck.append(balanced_ingredients[i].duplicate())
		elif i < good_ingredients.size():
			# If not enough balanced, add more good ones
			deck.append(good_ingredients[i % good_ingredients.size()].duplicate())
	
	# Shuffle the deck
	deck.shuffle()
	print("[FridgeManager] Initialized balanced deck with %d cards (%d good, %d balanced)" % [deck.size(), good_count, remaining_slots])
	_emit_deck_changed()
