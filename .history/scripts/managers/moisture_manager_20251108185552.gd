...
func setup(ingredient_1: IngredientModel, ingredient_2: IngredientModel, difficulty: float = 1.0, inventory: InventoryManager = null) -> void:
	...
	var total_volatility = ingredient_1.volatility + ingredient_2.volatility
	
	# Calculate base drain using combined stats
	var base_drain = 5.0
	var drain = base_drain + (total_volatility * 0.20) - (total_heat_resist * 0.035) - (total_density * 0.030)
	base_drain_rate = maxf(drain, 1.0)
	
	# PENALTY: Using same ingredient twice increases drain by 25% (no synergy)
	# BONUS: Using different ingredients reduces drain by 15% (synergy bonus)
	var synergy_modifier = ""
	if ingredient_1.name == ingredient_2.name:
		base_drain_rate *= 1.25
		synergy_modifier = " (25% penalty - same ingredient)"
	else:
		base_drain_rate *= 0.85
		synergy_modifier = " (15% synergy bonus - mixed ingredients)"
	
	# Debug output
	print("=== Moisture Setup ===")
	print("Ingredient 1: %s (Water: %d, Resist: %d, Density: %d, Volatility: %d)" % [
		ingredient_1.name, ingredient_1.water_content, ingredient_1.heat_resistance, 
		ingredient_1.density, ingredient_1.volatility])
	print("Ingredient 2: %s (Water: %d, Resist: %d, Density: %d, Volatility: %d)" % [
		ingredient_2.name, ingredient_2.water_content, ingredient_2.heat_resistance, 
		ingredient_2.density, ingredient_2.volatility])
	print("Total Stats - Water: %d, Resist: %d, Density: %d, Volatility: %d" % [
		total_water, total_heat_resist, total_density, total_volatility])
	print("Base drain rate: %.2f/sec%s" % [base_drain_rate, synergy_modifier])
