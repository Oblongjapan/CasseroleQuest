...
func _ready():
	...
	# Tier 1 is unlocked at game start (simple combos available immediately)
	tier_1_unlocked = true
	current_tier = 1
	print("[ProgressionManager] Tier 1 unlocked at start")
...
func _check_tier_unlocks() -> void:
	...
	print("[ProgressionManager] _check_tier_unlocks: total=%d, current_tier=%d, new_tier=%d" % [total_recipes_created, current_tier, new_tier])
