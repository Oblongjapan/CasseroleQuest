...
func _on_round_started(ingredient_1: IngredientModel, ingredient_2: IngredientModel) -> void:
	...
	# Initialize all managers with difficulty scaling
	moisture_manager.setup(ingredient_1, ingredient_2, difficulty_multiplier)
	var timer_duration = 15.0 / difficulty_multiplier  # Timer gets shorter each round
	timer_manager.start_timer(timer_duration)
