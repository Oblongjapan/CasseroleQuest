...
## Handle Make Organic button pressed - upgrade a random card with random stat
func _on_make_organic_pressed() -> void:
	...
	print("[Main] Selected random card for organic upgrade: %s" % random_card.name)
	...
	print("[Main] Organic upgrade: %s %+d" % [chosen_stat, amount])
	
	# Create modification dictionary
	var modifications: Dictionary = {chosen_stat: amount}
	
	# Apply upgrade to the card
	fridge_manager.upgrade_ingredient_stats(random_card.name, modifications)
	...
	match chosen_stat:
		"water":
			stat_name = "Water"
		"heat_resistance":
			stat_name = "Heat Resistance"
		"volatility":
			stat_name = "Volatility"
	
	var message = "Made %s Organic!\n\n%s %+d" % [random_card.name, stat_name, amount]
	_show_organic_message(message)
