...
func _on_ingredient_deselected(overlay_index: int) -> void:
	...
	if overlay:
		overlay.hide()
		overlay.texture = null  # Clear the texture so it can't be dragged again
		overlay.reset_position()

func _on_overlay_drag_out(overlay_index: int) -> void:
	print("[Main] Overlay slot %d dragged out - deselecting ingredient" % overlay_index)
	# Hide the overlay and clear texture
	var overlay = ingredient_overlay_1 if overlay_index == 0 else ingredient_overlay_2
	if overlay:
		overlay.hide()
		overlay.texture = null  # Clear texture to prevent double-dragging
		overlay.reset_position()
