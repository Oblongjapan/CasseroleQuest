...
func _on_sample_selected(index: int) -> void:
	...
	await _show_sample_animation(selected_ingredient)
...
func _show_sample_animation(ingredient: IngredientModel) -> void:
	...
	var start_pos = viewport_size / 2.0 - (card.size / 2.0)
	...
	# Phase 2: Slide to recipe box (fridge position)
	const RECIPE_BOX_POSITION := Vector2(641, 360)  # Center of fridge area
	...
	slide_tween.tween_property(card, "global_position", RECIPE_BOX_POSITION - (card.size / 2), 0.8)
...
func _on_card_selected_for_organic(ingredient_name: String) -> void:
	...
	if upgraded_ingredient:
		...
		await _show_organic_upgrade_animation(upgraded_ingredient)
	
	# Clear pending upgrade
	pending_organic_upgrade.clear()
	
## Show organic upgrade animation - spring in and slide to recipe box
func _show_organic_upgrade_animation(ingredient: IngredientModel) -> void:
