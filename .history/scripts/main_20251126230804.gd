...
@onready var sample_button_1: Button = $UI/ShopUI/SampleButton1
@onready var sample_button_2: Button = $UI/ShopUI/SampleButton2
@onready var sample_button_3: Button = $UI/ShopUI/SampleButton3
@onready var exit_shop_button: Button = $UI/ShopUI/ExitButton

const MAIN_CAMERA_POS = Vector2(640, 360)
const MAIN_CAMERA_ZOOM = Vector2(1.0, 1.0)
const SHOP_CAMERA_POS = Vector2(1000, 300) # Focused on fridge area
const SHOP_CAMERA_ZOOM = Vector2(1.5, 1.5)
...
func _on_shop_opened() -> void:
	...
	if main_camera:
		var tween = create_tween()
		tween.set_parallel(true)
		tween.tween_property(main_camera, "position", SHOP_CAMERA_POS, 1.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
		tween.tween_property(main_camera, "zoom", SHOP_CAMERA_ZOOM, 1.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
...
func _on_shop_closed() -> void:
	...
	if main_camera:
		var tween = create_tween()
		tween.set_parallel(true)
		tween.tween_property(main_camera, "position", MAIN_CAMERA_POS, 1.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
		tween.tween_property(main_camera, "zoom", MAIN_CAMERA_ZOOM, 1.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
		await tween.finished
...
func _populate_shop_buttons() -> void:
	...
	var buttons = [sample_button_1, sample_button_2, sample_button_3]
...
func _on_sample_selected(index: int) -> void:
	...
	# Disable button to show it's taken
	var buttons = [sample_button_1, sample_button_2, sample_button_3]
	if buttons[index]:
		buttons[index].disabled = true
		buttons[index].text = "Taken"
