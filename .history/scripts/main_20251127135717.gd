...
func _ready():
	...
	# Setup audio filters for shop
	if music_player:
		music_bus_index = AudioServer.get_bus_index("Music")
		if music_bus_index < 0:
			print("[Main] ERROR: Music bus not found!")
			music_bus_index = 0  # Fallback to master
		print("[Main] Music player using bus: %s (index %d)" % [music_player.bus, music_bus_index])
		...
		# The Music bus should have lowpass at index 0 and phaser at index 1
		lowpass_effect_index = 0
		phaser_effect_index = 1
		
		var effect_count = AudioServer.get_bus_effect_count(music_bus_index)
		print("[Main] Music bus has %d effects" % effect_count)
		
		# Verify effects exist
		if effect_count > 0:
			var effect0 = AudioServer.get_bus_effect(music_bus_index, 0)
			print("[Main] Effect 0: %s" % effect0)
		if effect_count > 1:
			var effect1 = AudioServer.get_bus_effect(music_bus_index, 1)
			print("[Main] Effect 1: %s" % effect1)
		...
		print("[Main] Audio effects initialized on Music bus and disabled")
	else:
