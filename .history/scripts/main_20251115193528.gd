...
@onready var map_ui: Panel = $UI/MapUI
...
func _update_ingredient_overlays(ingredient_1: IngredientModel, ingredient_2) -> void:
	...
	if ingredient_overlay_1 and ingredient_1:
		var texture_path = "res://Assets/Food/%s.png" % ingredient_1.name
	...
	if ingredient_overlay_2 and ingredient_2 and ingredient_2 != ingredient_1:
		var texture_path = "res://Assets/Food/%s.png" % ingredient_2.name
		var texture = load(texture_path)
		if texture:
...
func _on_game_started() -> void:
	...
	# Map system disabled for now - keeping code for later use
	# # Generate the progression map
	# print("[Main] Generating progression map...")
	# map_manager.generate_map(round_modifier_manager)
	# 
	# # Show the map for player to choose first node
	# print("[Main] Showing progression map...")
	# _show_map()
...
func _on_shop_closed() -> void:
	...
	print("[Main] Shop closed - returning to map")
	_check_map_progress()
...
## Show the progression map
func _show_map() -> void:
	_hide_all_ui()
	if map_ui:
		map_ui.show_map(map_manager)
		status_label.text = "Choose your path"
		print("[Main] Map displayed")
	else:
		print("[Main] ERROR: MapUI not found!")

## Called when player clicks a node on the map
func _on_map_node_clicked(node: MapNodeModel) -> void:
	print("[Main] Map node clicked: %s" % node.get_display_name())
	map_manager.select_node(node)

## Called when a node is selected (after map_manager processes it)
func _on_map_node_selected(node: MapNodeModel) -> void:
	print("[Main] Processing selected node: %s" % node.get_display_name())
	
	# Refresh map UI to show updated state
	if map_ui:
		map_ui.refresh_ui()
		map_ui.hide()
	...
	match node.type:
		MapNodeModel.NodeType.COOKING:
			# Regular cooking round - no modifier
			round_modifier_manager.set_modifier(null)
			_show_hand_selector()
		
		MapNodeModel.NodeType.MODIFIER:
			# Apply the modifier then cook
			if node.modifier:
				round_modifier_manager.set_modifier(node.modifier)
				if round_modifier_label:
					round_modifier_label.set_modifier(node.modifier)
			_show_hand_selector()
		...
		MapNodeModel.NodeType.REST:
			# Show upgrade selection (reuse shop with special mode)
			_show_upgrade_station()
		
		MapNodeModel.NodeType.BOSS:
			# Boss round - harder modifier
			var boss_modifier = RoundModifierModel.new()
			boss_modifier.name = "Boss Challenge"
			boss_modifier.description = "Drain rate increased by 50%"
			boss_modifier.modifier_type = RoundModifierModel.ModifierType.DRAIN_MULTIPLIER
			boss_modifier.value = 1.5
			round_modifier_manager.set_modifier(boss_modifier)
			if round_modifier_label:
				round_modifier_label.set_modifier(boss_modifier)
			_show_hand_selector()
		
		MapNodeModel.NodeType.MALFUNCTION:
			# Trigger malfunction event
			_trigger_malfunction_event()

## Show upgrade station (simplified - just advance for now)
func _show_upgrade_station() -> void:
	print("[Main] Upgrade station - advancing map")
	# TODO: Implement card upgrade UI
	await get_tree().create_timer(1.0).timeout
	_check_map_progress()

## Trigger a malfunction event
func _trigger_malfunction_event() -> void:
	print("[Main] Malfunction event triggered!")
	# Show malfunction popup with the existing system
	malfunction_manager.trigger_malfunction()
	# The malfunction popup will handle showing rewards/effects
	# After closing, return to map
	await get_tree().create_timer(0.5).timeout
	_check_map_progress()

## Check if map is complete or show next choices
func _check_map_progress() -> void:
	# Map system disabled - continue to next round directly
	print("[Main] Round complete - continuing to next round")
	current_round_number += 1
	_show_hand_selector()
	
	# Old map-based logic (keeping for later):
	# if map_manager.is_map_complete():
	# 	print("[Main] Map complete - Victory!")
	# 	_show_game_over()
	# else:
	# 	_show_map()
