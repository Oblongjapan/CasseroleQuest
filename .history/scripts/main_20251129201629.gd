...
func _show_upgrade_highlight(card: Control, upgrade_data: Dictionary) -> void:
	...
	for key in upgrade_data.keys():
		...
		match key:
			"water":
				upgrade_text = "Water %+d" % amount
				stat_color = Color(0.3, 0.7, 1.0)  # Blue for water
			"heat_resistance":
				upgrade_text = "Heat Resist %+d" % amount
				stat_color = Color(1.0, 0.5, 0.2)  # Orange for heat
			"volatility":
				upgrade_text = "Volatility %+d" % amount
	...
	highlight_label.add_theme_font_size_override("font_size", 32)
	...
	highlight_label.add_theme_constant_override("outline_size", 4)
	...
	card.add_child(highlight_label)
	highlight_label.position = Vector2(card.size.x / 2 - 100, card.size.y + 10)
	highlight_label.size = Vector2(200, 50)
