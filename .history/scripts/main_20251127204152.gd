...
func _on_round_started(ingredient_1: IngredientModel, ingredient_2) -> void:
	...
	# Scale difficulty based on tier progression (more aggressive scaling)
	# Tier 1: 1.0x (base difficulty)
	# Tier 2: 2.0x (100% harder - double drain rate)
	# Tier 3: 2.5x (150% harder)
	# Tier 4: 3.0x (200% harder)
	# Tier 5: 3.5x (250% harder)
	var current_tier = progression_manager.current_tier
	var difficulty_multiplier = 1.0
	if current_tier >= 2:
		difficulty_multiplier = 1.5 + (current_tier * 0.5)
...
func _show_recipe_notification() -> void:
	...
	if recipe_card_reveal and last_created_recipe:
		...
		if moisture_label:
			moisture_label.text = "0/0"
			print("[Main] Reset moisture label to 0/0")
	else:
