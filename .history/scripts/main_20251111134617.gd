...
func _ready():
	...
	if speed_button:
		...
		print("[Main] Speed button connected")
	else:
		print("[Main] Warning: Speed button not found")
...
func _on_ingredient_selected(ingredient: IngredientModel, index: int) -> void:
	print("[Main] Ingredient selected: %s at index %d" % [ingredient.name, index])
	var overlay = ingredient_overlay_1 if index == 0 else ingredient_overlay_2
	if overlay:
		...
		if texture:
			...
			overlay.show()
			print("[Main] Showing ingredient overlay %d: %s" % [index + 1, ingredient.name])

func _on_ingredient_deselected(index: int) -> void:
	print("[Main] Ingredient deselected at index %d" % index)
	var overlay = ingredient_overlay_1 if index == 0 else ingredient_overlay_2
	if overlay:
		overlay.hide()

func _on_overlay_drag_out(overlay_index: int) -> void:
	print("[Main] Overlay %d dragged out - deselecting ingredient" % overlay_index)
	# Hide the overlay
	...
	# Tell hand selector to deselect this ingredient
	if hand_selector and hand_selector.visible:
		hand_selector.deselect_by_index(overlay_index)
...
func _toggle_speed() -> void:
	...
	print("[Main] Speed changed to: %.1fx" % game_speed)
