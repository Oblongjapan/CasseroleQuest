...
func _show_organic_upgrade_animation(ingredient: IngredientModel, start_pos: Vector2) -> void:
	...
	# Phase 2: Show upgrade highlight for 2 seconds
	await _show_upgrade_highlight(card, pending_organic_upgrade)
...
## Show upgrade highlight on card - creates a glowing label that pulses
func _show_upgrade_highlight(card: Control, upgrade_data: Dictionary) -> void:
	print("[Main] Showing upgrade highlight")
	
	# Determine which stat was upgraded and format the text
	var upgrade_text = ""
	var stat_color = Color.GREEN
	var target_label_name = ""
	
	for key in upgrade_data.keys():
		var amount = upgrade_data[key]
		match key:
			"water":
				upgrade_text = "+%d" % amount
				stat_color = Color(0.3, 0.7, 1.0)  # Blue for water
				target_label_name = "WaterLabel"
			"heat_resistance":
				upgrade_text = "+%d" % amount
				stat_color = Color(1.0, 0.5, 0.2)  # Orange for heat
				target_label_name = "ResistLabel"
			"volatility":
				upgrade_text = "%+d" % amount
				stat_color = Color(0.9, 0.3, 0.9)  # Purple for volatility
				target_label_name = "VolatilityLabel"
	
	# Find the specific stat label to overlay
	...
	highlight_label.text = "⬆ %s ⬆" % upgrade_text
	highlight_label.add_theme_font_size_override("font_size", 20)
	highlight_label.add_theme_color_override("font_color", stat_color)
	highlight_label.add_theme_color_override("font_outline_color", Color.BLACK)
	highlight_label.add_theme_constant_override("outline_size", 3)
	...
	# Position directly over the stat label
	target_label.add_child(highlight_label)
	highlight_label.anchors_preset = Control.PRESET_FULL_RECT
	highlight_label.anchor_left = 0.0
	highlight_label.anchor_top = 0.0
	highlight_label.anchor_right = 1.0
	highlight_label.anchor_bottom = 1.0
	highlight_label.offset_left = 0
	highlight_label.offset_top = 0
	highlight_label.offset_right = 0
	highlight_label.offset_bottom = 0
	highlight_label.z_index = 100
	
	# Pulse animation
	highlight_label.modulate.a = 0.0
	var pulse_tween = create_tween()
	pulse_tween.set_loops(4)  # Pulse 4 times over 2 seconds
	pulse_tween.tween_property(highlight_label, "modulate:a", 1.0, 0.25)
	pulse_tween.tween_property(highlight_label, "modulate:a", 0.6, 0.25)
	
	# Hold for 2 seconds total
	await get_tree().create_timer(2.0).timeout
	
	# Fade out the label
	if highlight_label and is_instance_valid(highlight_label):
		highlight_label.queue_free()
	
	print("[Main] Upgrade highlight complete")
