...
func _on_round_started(ingredient_1: IngredientModel, ingredient_2) -> void:
	...
	# Change microwave background to "on" asset if available
	var on_tex = load("res://Assets/Microwave_ON.png")
	if on_tex:
		if has_node("UI/Background"):
			get_node("UI/Background").texture = on_tex
		else:
			print("UI/Background node not found to set microwave ON texture")
	else:
		print("Microwave_ON.png not found in Assets; background unchanged")
...
func _end_round(success: bool) -> void:
	...
	# Restore microwave background to default (off) asset if available
	...
	if success:
		...
		if draft_selector:
			# Pass current round number (before incrementing) to determine draft type
			draft_selector.show_draft(inventory_manager, false, current_round_number)
...
func _calculate_difficulty() -> void:
	# Difficulty increases exponentially: combines linear growth with a small exponential factor
	# Round 1: 1.0x, Round 2: 1.12x, Round 3: 1.26x, Round 4: 1.42x, etc.
	var base_scaling = 0.1
	var exponential_factor = 0.02  # Adds compound growth
	...
	var adjusted_exponential = exponential_factor * (1.0 - scaling_reduction)
	
	var rounds_passed = current_round_number - 1
	difficulty_multiplier = 1.0 + (rounds_passed * adjusted_scaling) + (rounds_passed * rounds_passed * adjusted_exponential)
