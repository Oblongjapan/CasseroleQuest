...
func _check_and_create_recipe() -> void:
	...
	if combined_ingredient:
		...
		if current_round_ingredients.size() > RecipesData.TIER_1_MAX_INGREDIENTS and current_tier < 2:
			print("[Main] ⚠️ TIER RESTRICTION: Need Tier 2 to create %d-ingredient recipes!" % current_round_ingredients.size())
			print("[Main] ⚠️ Create %d more unique 2-ingredient recipes to unlock Tier 2" % (RecipesData.TIER_2_UNLOCK_THRESHOLD - progression_manager.unique_recipes_created))
			# TODO: Show UI notification to player about tier restriction
