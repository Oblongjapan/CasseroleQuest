...
func _ready():
	...
	if music_player:
		...
		# Pre-create audio effects for HTML5 compatibility
		...
		print("[Main] Audio effects pre-created for shop")
	else:
...
func _apply_shop_music_filter() -> void:
	...
	for i in range(effect_count - 1, -1, -1):
		...
		if effect is AudioEffectLowPassFilter or effect is AudioEffectPhaser:
			...
			print("[Main] Removed existing filter at index %d" % i)
	
	# Always recreate and add filters fresh for HTML5 compatibility
	low_pass_filter = AudioEffectLowPassFilter.new()
	low_pass_filter.cutoff_hz = 800.0
	low_pass_filter.resonance = 1.0
	AudioServer.add_bus_effect(music_bus_index, low_pass_filter)
	print("[Main] Added low pass filter to shop music")
	...
	print("[Main] Added phaser filter to shop music")
	
	# Verify filters were added
	var new_effect_count = AudioServer.get_bus_effect_count(music_bus_index)
	print("[Main] Bus now has %d effects" % new_effect_count)
