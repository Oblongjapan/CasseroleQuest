...
func _show_recipe_notification() -> void:
	...
	# Create recipe notification popup with proper layering
	print("[Main] Creating AcceptDialog popup")
	var popup = AcceptDialog.new()
	popup.dialog_text = "%s created!" % recipe.name
	popup.title = "Recipe Discovered"
	popup.initial_position = Window.WINDOW_INITIAL_POSITION_ABSOLUTE
	popup.position = Vector2i(340, 460)  # Top-left corner at (340, 460)
	...
	popup.exclusive = true  # Make it modal
	popup.unresizable = true
	popup.always_on_top = true  # Keep it on top
	
	# Add to root instead of UI layer (Windows must be added to the scene tree root or a viewport)
	get_tree().root.add_child(popup)
	print("[Main] Popup added to root")
	popup.popup_centered()
	print("[Main] popup_centered() called. Popup visible: %s, position: %s, size: %s" % [popup.visible, popup.position, popup.size])
	
	print("[Main] Showing recipe popup for: %s - awaiting confirmation" % recipe.name)
	
	# Wait for user to click OK
	await popup.confirmed
	
	print("[Main] Recipe popup confirmed")
