...
func _show_recipe_notification() -> void:
	...
	if not recipe_created_this_round:
	...
	# Get all individual ingredient names for sprite display
	...
	# Get the recipe name from RecipesData (this will be the unique dish name)
	var recipe_name = RecipesData.get_recipe_name_for_ingredients(all_ingredient_names)
	...
	# Show the combo recipe in the microwave overlays (on plates with ingredient sprites)
	_display_combo_in_microwave(all_ingredient_names)
