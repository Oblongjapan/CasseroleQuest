...
func _update_ingredient_overlays(ingredient_1: IngredientModel, ingredient_2) -> void:
	...
	if ingredient_overlay_1 and ingredient_1:
		...
		if "+" in base_name:
			# For combo recipes, use the first ingredient's texture
			var ingredient_names = base_name.split("+")
			if ingredient_names.size() > 0:
				var first_ingredient = ingredient_names[0]
				var texture_path = "res://Assets/Food/%s.png" % first_ingredient
				var texture = load(texture_path)
				if texture:
					ingredient_overlay_1.set_ingredient(ingredient_1.name, texture)
					print("[Main] Showing ingredient overlay 1: %s (combo, using %s)" % [ingredient_1.name, first_ingredient])
				else:
					print("[Main] WARNING: Could not load texture for combo ingredient 1: %s" % texture_path)
		else:
	...
	if ingredient_overlay_2 and ingredient_2 and ingredient_2 != ingredient_1:
		...
		if "+" in base_name:
			...
			if ingredient_names.size() > 0:
				...
				var texture = load(texture_path)
				if texture:
					ingredient_overlay_2.set_ingredient(ingredient_2.name, texture)
					print("[Main] Showing ingredient overlay 2: %s (combo, using %s)" % [ingredient_2.name, first_ingredient])
				else:
					print("[Main] WARNING: Could not load texture for combo ingredient 2: %s" % texture_path)
		else:
			# Single ingredient - normal handling
			# Map recipe names to actual PNG filenames
			var recipe_name_map = {
...
func _on_ingredient_selected(ingredient: IngredientModel, overlay_index: int) -> void:
	...
	if overlay:
		...
		if "+" in base_name:
			...
			# For combo recipes, just use the first ingredient's texture
			# The overlay will show it on a plate
			var ingredient_names = base_name.split("+")
			if ingredient_names.size() > 0:
				var first_ingredient = ingredient_names[0]
				var texture_path = "res://Assets/Food/%s.png" % first_ingredient
				...
				if texture:
					...
					print("[Main] Showing combo ingredient overlay %d: %s (using %s texture)" % [overlay_index + 1, ingredient.name, first_ingredient])
				else:
					print("[Main] WARNING: Could not load texture: %s" % texture_path)
			else:
				print("[Main] ERROR: Could not parse combo recipe name: %s" % base_name)
		else:
			# Single ingredient - normal handling
			# Map recipe names to actual PNG filenames
			var recipe_name_map = {
...
func _check_and_create_recipe() -> void:
	...
	if combined_ingredient:
		...
		print("[Main] Original ingredients REMOVED from circulation")
	else:
...
func _show_recipe_notification() -> void:
	...
	# For dynamic recipes, we won't show a sprite (since we don't have pre-made combo images)
	# Just show the notification popup
