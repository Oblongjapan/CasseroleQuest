...
@onready var recipe_discovery_reveal: Control = $UI/RecipeDiscoveryReveal
@onready var card_selector: CardSelector = $UI/CardSelector
@onready var discovery_notification: Label = $UI/DiscoveryNotification
...
@onready var radio_animation: AnimationPlayer = $UI/RadioOverlay/AnimationPlayer
...
var music_enabled: bool = true  # Track if music is playing
...
# Mobile input tracking for long-press pause
var touch_start_time: float = 0.0
var touch_position: Vector2 = Vector2.ZERO
var is_long_pressing: bool = false
const LONG_PRESS_DURATION: float = 0.5  # 500ms to trigger pause
...
func _ready():
	...
	# Connect recipe book manager signals
	if recipe_book_manager:
		recipe_book_manager.recipe_discovered.connect(_on_recipe_discovered)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_S and not shop_ui.visible:
			_open_shop(false)
			get_viewport().set_input_as_handled()
			return
		
		# Debug: Press F to show discovery reveal screen
		if event.keycode == KEY_F:
			_debug_show_discovery_reveal()
			get_viewport().set_input_as_handled()
			return
...
func _process(delta: float) -> void:
	# Check for long press to trigger pause (mobile)
	if touch_start_time > 0 and not is_long_pressing:
		var current_time = Time.get_ticks_msec() / 1000.0
		if current_time - touch_start_time >= LONG_PRESS_DURATION:
			is_long_pressing = true
			touch_start_time = 0.0
			
			# Trigger pause menu
			if pause_menu and not pause_menu.visible:
				print("[Main] Long press detected - opening pause menu")
				_apply_pause_music_filter()
				pause_menu.show_pause_menu()
	
	# Only process if round is active (skip during menus, shops, etc)
...
func _on_game_started() -> void:
	...
	recipe_book_manager.clear_session_discoveries()  # Clear discoveries from previous run
...
func _show_game_over() -> void:
	...
	# Check if there are new discoveries to show
	var session_discoveries = recipe_book_manager.get_session_discoveries()
	if session_discoveries.size() > 0 and recipe_discovery_reveal:
		print("[Main] Showing %d new discoveries before game over" % session_discoveries.size())
		recipe_discovery_reveal.show_discoveries(session_discoveries, func():
			# After all discoveries shown, show game over screen
			_show_game_over_screen()
		)
	else:
		# No discoveries, go straight to game over
		_show_game_over_screen()
...
## Show notification when a new recipe is discovered
func _on_recipe_discovered(_recipe_identity: String, display_name: String) -> void:
	if not discovery_notification:
		return
	
	print("[Main] 🎉 New recipe discovered: %s" % display_name)
	
	# Set the text
	discovery_notification.text = "NEW RECIPE: %s" % display_name
	
	# Make visible and animate
	discovery_notification.modulate.a = 0.0
	discovery_notification.show()
	
	# Fade in
	var tween = create_tween()
	tween.tween_property(discovery_notification, "modulate:a", 1.0, 0.3)
	tween.tween_interval(2.5)  # Stay visible for 2.5 seconds
	tween.tween_property(discovery_notification, "modulate:a", 0.0, 0.5)
	tween.tween_callback(func(): discovery_notification.hide())

## Debug: Show discovery reveal screen with sample data
func _debug_show_discovery_reveal() -> void:
	print("[Main] Debug: Showing discovery reveal screen")
	
	# Create some sample discoveries
	var test_discoveries: Array[Dictionary] = [
		{
			"identity": "Chicken Breast+Rice",
			"display_name": "Chicken Rice Bowl",
			"discovered_at": Time.get_unix_time_from_system()
		},
		{
			"identity": "Broccoli+Carrot+Potato",
			"display_name": "Garden Medley",
			"discovered_at": Time.get_unix_time_from_system()
		},
		{
			"identity": "Salmon+Spinach",
			"display_name": "Power Plate",
			"discovered_at": Time.get_unix_time_from_system()
		}
	]
	
	_hide_all_ui()
	if recipe_discovery_reveal:
		recipe_discovery_reveal.show_discoveries(test_discoveries, func():
			print("[Main] Debug: Discovery reveal completed")
			_hide_all_ui()
			main_menu.show()
		)
...
func _on_shop_opened() -> void:
	...
	# Reset organic upgrade usage and generate new upgrade stats
	organic_used = false
	if make_organic_button:
		make_organic_button.disabled = false
		make_organic_button.text = "Make Organic"
	
	# Generate the organic upgrade stats once for this shop phase
	_generate_organic_upgrade_stats()
...
func _on_shop_closed() -> void:
	...
	_show_hand_selector(true)## Handle radio button pressed - toggle music on/off
...
## Show a tier restriction message when player tries to combine ingredients before unlocking that tier
func _show_tier_restriction_message(ingredient_count: int, recipes_needed: int) -> void:
	# Create a simple popup dialog
	var popup = AcceptDialog.new()
	popup.dialog_text = "⚠️ PROGRESSION LOCKED: %d-Ingredient Recipes\n\nYou need Tier 2 progression to create recipes with %d or more ingredients.\n\nCreate %d more unique 2-ingredient recipes to unlock!" % [ingredient_count, ingredient_count, recipes_needed]
	add_child(popup)
...
## Generate organic upgrade stats (called once when shop opens)
func _generate_organic_upgrade_stats() -> void:
	...
	# Store the upgrade data for this shop phase
	pending_organic_upgrade = {chosen_stat: amount}
	print("[Main] Generated organic upgrade for shop phase: %s %+d" % [chosen_stat, amount])
...
func _on_make_organic_pressed() -> void:
	...
	# Use the pre-generated upgrade stats (generated when shop opened)
	if pending_organic_upgrade.is_empty():
		print("[Main] ERROR: No organic upgrade generated for this shop phase")
		return
