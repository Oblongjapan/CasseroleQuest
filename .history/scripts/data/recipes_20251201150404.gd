...
func get_recipe_name_for_ingredients(ingredient_names: Array[String]) -> String:
	...
	# ===== 2 INGREDIENTS: BASE + PREMIUM =====
	# 8 base × 5 premium = 40 combinations
	
	# Asparagus combinations (8)
	"Asparagus|Bread": "Asparagus Toast",
	"Asparagus|Broccoli": "Green Stalk Medley",
	"Asparagus|Carrot": "Fancy Vegetables",
	"Asparagus|Chicken Breast": "Chicken Asparagus",
	"Asparagus|Lettuce": "Asparagus Salad",
	"Asparagus|Peas": "Premium Greens",
	"Asparagus|Potato": "Asparagus & Potato",
	"Asparagus|Rice": "Asparagus Rice",
	
	# Salmon combinations (8)
	"Bread|Salmon": "Salmon Sandwich",
	"Broccoli|Salmon": "Salmon & Broccoli",
	"Carrot|Salmon": "Salmon with Carrots",
	"Chicken Breast|Salmon": "Surf & Turf Lite",
	"Lettuce|Salmon": "Salmon Salad",
	"Peas|Salmon": "Salmon & Peas",
	"Potato|Salmon": "Salmon Potato",
	"Rice|Salmon": "Salmon Rice Bowl",
	
	# Spinach combinations (8)
	"Bread|Spinach": "Spinach Sandwich",
	"Broccoli|Spinach": "Super Greens",
	"Carrot|Spinach": "Garden Medley",
	"Chicken Breast|Spinach": "Chicken Spinach",
	"Lettuce|Spinach": "Leafy Salad",
	"Peas|Spinach": "Green Power",
	"Potato|Spinach": "Spinach Potato",
	"Rice|Spinach": "Spinach Rice",
	
	# Steak combinations (8)
	"Bread|Steak": "Steak Sandwich",
	"Broccoli|Steak": "Steak & Broccoli",
	"Carrot|Steak": "Steak with Carrots",
	"Chicken Breast|Steak": "Mixed Meats",
	"Lettuce|Steak": "Steak Salad",
	"Peas|Steak": "Steak & Peas",
	"Potato|Steak": "Steak & Potatoes",
	"Rice|Steak": "Steak Rice Bowl",
	
	# Tofu combinations (8)
	"Bread|Tofu": "Tofu Sandwich",
	"Broccoli|Tofu": "Tofu Broccoli",
	"Carrot|Tofu": "Tofu with Carrots",
	"Chicken Breast|Tofu": "Protein Fusion",
	"Lettuce|Tofu": "Tofu Salad",
	"Peas|Tofu": "Tofu & Peas",
	"Potato|Tofu": "Tofu Potato",
	"Rice|Tofu": "Tofu Rice Bowl",
	
	# ===== 3 INGREDIENTS =====
	# Hundreds of combinations with base ingredients
	# Plus combinations with 1 premium ingredient (Asparagus, Salmon, Spinach, Steak, Tofu)		# 3 Ingredients - Base only (many combinations)
	...
	# 9 Ingredients
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice": "Epic Kitchen Sink",
	"Asparagus|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Salmon|Spinach|Tofu": "Epic Protein Overload",
	"Bread|Broccoli|Carrot|Chicken Breast|Peas|Potato|Rice|Steak|Tofu": "Epic Carb-Protein Fusion",
	"Asparagus|Broccoli|Carrot|Lettuce|Peas|Potato|Rice|Salmon|Steak": "Epic Ocean & Land",
	
	# ===== 10 INGREDIENTS =====
	# (Mythic combinations)
	
	# 10 Ingredients
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon": "Mythic Abundance Bowl",
	"Asparagus|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Spinach|Steak|Tofu": "Mythic Power Feast",
	"Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Rice|Salmon|Spinach|Steak": "Mythic Everything Sandwich",
	"Asparagus|Broccoli|Carrot|Lettuce|Peas|Potato|Rice|Salmon|Spinach|Tofu": "Mythic Garden of Eden",
	
	# ===== 11 INGREDIENTS =====
	# (Godlike combinations)
	
	# 11 Ingredients
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Spinach": "Godlike Perfection Bowl",
	"Asparagus|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Steak|Tofu": "Godlike Protein Kingdom",
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Spinach|Steak|Tofu": "Godlike Ultimate Feast",
	"Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Spinach|Steak": "Godlike Master's Choice",
	
	# ===== 12 INGREDIENTS =====
	# (Transcendent combinations)
	
	# 12 Ingredients
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Spinach|Steak": "Transcendent Harmony",
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Steak|Tofu": "Transcendent Unity Bowl",
	
	# ===== 13 INGREDIENTS =====
	# (THE ULTIMATE RECIPE - All ingredients together!)
	
	# 13 Ingredients - THE ONE
	"Asparagus|Bread|Broccoli|Carrot|Chicken Breast|Lettuce|Peas|Potato|Rice|Salmon|Spinach|Steak|Tofu": "Last Supper"
...
func combine_ingredients(ingredients: Array[IngredientModel], current_tier: int = 1) -> IngredientModel:
	...
	# IMPORTANT: Use CURRENT stats from ingredient cards (including upgrades), not base stats
	var unique_base_ingredients: Dictionary = {}  # name -> IngredientModel
	var ingredient_card_stats: Dictionary = {}  # Track which card each ingredient came from
	
	# First pass: Map ingredient names to their actual card instances
	for ingredient in ingredients:
		# Split by "+" to handle combined ingredients
		var parts = ingredient.name.split("+")
		if parts.size() == 1:
			# Single ingredient card - use its current stats directly
			var clean_name = parts[0].strip_edges()
			if not ingredient_card_stats.has(clean_name):
				ingredient_card_stats[clean_name] = ingredient
		else:
			# Combined card - we need to break it down to base ingredients
			# For combined cards, we'll use base stats since we can't track individual upgrades
			for part in parts:
				var clean_name = part.strip_edges()
				if not ingredient_card_stats.has(clean_name):
					# Use base stats for ingredients that are part of a combo
					var base_ing = get_base_ingredient_by_name(clean_name)
					if base_ing:
						ingredient_card_stats[clean_name] = base_ing
	
	# Second pass: Build unique_base_ingredients using actual card stats
	for ingredient in ingredients:
		var parts = ingredient.name.split("+")
		for part in parts:
			var clean_name = part.strip_edges()
			if not unique_base_ingredients.has(clean_name):
				if ingredient_card_stats.has(clean_name):
					var card_stats = ingredient_card_stats[clean_name]
					unique_base_ingredients[clean_name] = card_stats
					print("[RecipesData]   Found unique base: %s (Water:%d, RST:%d, Vol:%d)" % [
						clean_name, card_stats.water_content, card_stats.heat_resistance, card_stats.volatility
					])
	
	# Calculate stats based on unique base ingredients with CURRENT stats (including upgrades)
	var total_water = 0
...
func combine_ingredients_from_names(ingredient_names: Array) -> IngredientModel:
	print("[RecipesData] combine_ingredients_from_names called with: %s" % ", ".join(ingredient_names))
	
	# Create ingredient models from names
	var ingredients: Array[IngredientModel] = []
	for ing_name in ingredient_names:
		var clean_name = ing_name.strip_edges()
		print("[RecipesData] Looking up ingredient: '%s'" % clean_name)
		
		# Check if it's already a combo (contains +)
		if "+" in clean_name:
			# Parse combo ingredients
			var combo_parts = clean_name.split("+")
			var combo_ingredients: Array[IngredientModel] = []
			for part in combo_parts:
				var part_clean = part.strip_edges()
				# Try to find the ingredient by matching display name
				var found = false
				for key in IngredientsData.INGREDIENTS.keys():
					var ing_data = IngredientsData.INGREDIENTS[key]
					if ing_data.name == part_clean:
						combo_ingredients.append(IngredientModel.new(part_clean, ing_data.water_content, ing_data.heat_resistance, ing_data.volatility))
						found = true
						break
				if not found:
					print("[RecipesData] WARNING: Ingredient '%s' not found" % part_clean)
			# Combine them
			var combo = combine_ingredients(combo_ingredients, 8)  # Use max tier to avoid restrictions
			if combo:
				ingredients.append(combo)
		else:
			# Single ingredient - find by display name
			var found = false
			for key in IngredientsData.INGREDIENTS.keys():
				var ing_data = IngredientsData.INGREDIENTS[key]
				if ing_data.name == clean_name:
					print("[RecipesData] Found ingredient '%s' (key: %s)" % [clean_name, key])
					ingredients.append(IngredientModel.new(clean_name, ing_data.water_content, ing_data.heat_resistance, ing_data.volatility))
					found = true
					break
			if not found:
				print("[RecipesData] WARNING: Ingredient '%s' not found in IngredientsData.INGREDIENTS" % clean_name)
	
	if ingredients.is_empty():
		print("[RecipesData] ERROR: No valid ingredients found")
		return null
	
	print("[RecipesData] Successfully created %d ingredient models" % ingredients.size())
	# Combine with max tier to avoid restrictions
	return combine_ingredients(ingredients, 8)
