...
func get_base_ingredient_by_name(ingredient_name: String) -> IngredientModel:
	...
	# Try to find in base ingredients first
	for base_name in BASE_INGREDIENTS:
		if base_name == clean_name:
			return IngredientsData.get_ingredient(base_name)
	
	# Try premium ingredients
	for premium_name in PREMIUM_INGREDIENTS:
		if premium_name == clean_name:
			return IngredientsData.get_ingredient(premium_name)
	
	# Fallback: try direct lookup
	return IngredientsData.get_ingredient(clean_name)
