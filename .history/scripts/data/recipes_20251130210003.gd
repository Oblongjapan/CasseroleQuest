...
## Smart recipe name generator for combinations without predefined names
## Analyzes ingredients to create contextual, thematic names
func _generate_contextual_recipe_name(ingredients: Array[String]) -> String:
	var count = ingredients.size()
	
	# Analyze ingredient composition
	var has_bread = "Bread" in ingredients
	var has_rice = "Rice" in ingredients
	var proteins = []
	var veggies = []
	var greens = []
	...
	for ing in ingredients:
		match ing:
			"Chicken Breast", "Steak", "Salmon", "Tofu":
				proteins.append(ing)
			"Lettuce", "Spinach", "Broccoli", "Peas", "Asparagus":
				greens.append(ing)
			"Carrot", "Potato":
				veggies.append(ing)
	
	# Base format selection based on ingredients
	var base_format = ""
	if has_bread:
		base_format = "Sandwich"
	elif has_rice:
		base_format = "Bowl"
	elif greens.size() >= count * 0.6:  # Mostly greens
		base_format = "Salad"
	elif proteins.size() >= 2:
		base_format = "Plate"
	else:
		base_format = "Bowl"
	
	# Generate prefix based on count and composition
	var prefix = ""
	
	# 4 ingredients - Mid-game creativity
	if count == 4:
		if proteins.size() >= 2:
			prefix = ["Mixed Protein", "Double Protein", "Protein Fusion", "Hearty"].pick_random()
		elif greens.size() >= 3:
			prefix = ["Garden Fresh", "Green Medley", "Veggie Blend", "Fresh Garden"].pick_random()
		elif has_bread and proteins.size() >= 1:
			prefix = ["Loaded", "Stacked", "Hearty", "Premium"].pick_random()
		else:
			prefix = ["Mixed", "Fusion", "Combo", "Medley"].pick_random()
	
	# 5-6 ingredients - Advanced recipes
	elif count == 5 or count == 6:
		if proteins.size() >= 3:
			prefix = ["Ultimate Protein", "Power Protein", "Mega Protein", "Triple Threat"].pick_random()
		elif greens.size() >= 4:
			prefix = ["Super Green", "Garden Supreme", "Mega Veggie", "Greens Galore"].pick_random()
		elif proteins.size() >= 2 and greens.size() >= 2:
			prefix = ["Balanced Power", "Complete", "Nutritionist's Dream", "Gourmet"].pick_random()
		else:
			var adjectives = ["Supreme", "Deluxe", "Grand", "Premium", "Special", "Signature"]
			prefix = adjectives.pick_random()
	
	# 7-8 ingredients - Epic late-game
	elif count == 7 or count == 8:
		var epic_prefixes = [
			"Kitchen Sink", "Everything", "Ultimate", "Legendary",
			"Epic", "Extreme", "Maximum", "Mega", "Master Chef's",
			"The Experiment", "Culinary Chaos", "Fusion Frenzy"
		]
		prefix = epic_prefixes.pick_random()
	
	# 9+ ingredients - Absurdist endgame
	elif count >= 9:
		if count == 13:  # THE ULTIMATE
			return ["The Perfect Fusion", "The Ultimate Casserole", "Microwave Singularity", 
					"The Forbidden Recipe", "Everything at Once"].pick_random()
		elif count >= 11:  # Near complete
			var near_complete = [
				"The Almost-Complete Feast", "Nearly Everything Bowl", 
				"Pre-Singularity Mix", "The Penultimate Plate",
				"What Have I Done", "Too Much of Everything"
			]
			return near_complete.pick_random()
		else:  # 9-10 ingredients
			var chaos_names = [
				"Ingredient Overload", "Microwave Madness Bowl",
				"The Chaos Casserole", "Everything But The...",
				"Desperation Dinner", "The Experiment Gone Right",
				"Fusion Confusion", "Maximum Chaos"
			]
			return chaos_names.pick_random()
	
	# Default for edge cases
	else:
		prefix = "Mixed"
	
	# Combine prefix with base format
	return prefix + " " + base_format
...
func get_recipe_name_for_ingredients(ingredient_names: Array[String]) -> String:
	...
	var recipe_names = {
		...
		# ===== 4 INGREDIENTS =====
		# Popular and interesting 4-ingredient combinations
		
		# Bread-based 4-ingredient recipes
		"Bread|Chicken Breast|Lettuce|Steak": "Ultimate Meat Sandwich",
		"Bread|Lettuce|Salmon|Spinach": "Healthy Fish Sandwich",
		"Bread|Carrot|Lettuce|Tofu": "Garden Veggie Sub",
		"Bread|Broccoli|Chicken Breast|Rice": "Chicken Teriyaki Wrap",
		
		# Bowl-based protein combos
		"Chicken Breast|Peas|Rice|Salmon": "Surf & Turf Rice Bowl",
		"Broccoli|Chicken Breast|Rice|Spinach": "Healthy Power Bowl",
		"Carrot|Peas|Potato|Rice": "Rustic Veggie Bowl",
		"Chicken Breast|Rice|Steak|Tofu": "Triple Protein Bowl",
		
		# Salad combinations
		"Broccoli|Carrot|Lettuce|Spinach": "Super Green Salad",
		"Carrot|Lettuce|Peas|Spinach": "Spring Garden Salad",
		"Chicken Breast|Lettuce|Spinach|Steak": "Power Meat Salad",
		
		# Veggie-heavy combos
		"Asparagus|Broccoli|Carrot|Peas": "Garden Four Medley",
		"Broccoli|Carrot|Potato|Spinach": "Roasted Veggie Mix",
		"Asparagus|Broccoli|Peas|Spinach": "All-Green Special",
		
		# Protein-focused
		"Chicken Breast|Salmon|Steak|Tofu": "Quad Protein Plate",
		"Chicken Breast|Potato|Salmon|Steak": "Fisherman's Feast",
		
		# Creative combos
		"Asparagus|Potato|Salmon|Spinach": "Healthy Catch Bowl",
		"Broccoli|Lettuce|Salmon|Tofu": "Ocean Garden Bowl",
		"Bread|Peas|Potato|Steak": "Hearty Steak Sub",
		"Carrot|Chicken Breast|Peas|Spinach": "Chicken Garden Stir-Fry",
		
		# ===== 5 INGREDIENTS =====
		# Curated 5-ingredient recipes
		
		# Already defined above, adding more interesting ones:
		"Bread|Chicken Breast|Lettuce|Salmon|Spinach": "Omega Power Sandwich",
		"Broccoli|Carrot|Lettuce|Peas|Spinach": "Ultimate Garden Salad",
		"Asparagus|Chicken Breast|Peas|Rice|Spinach": "Healthy Chicken Stir-Fry",
		"Bread|Carrot|Potato|Steak|Tofu": "Root & Protein Sub",
		"Chicken Breast|Peas|Potato|Rice|Steak": "Hearty Meat & Carbs",
		"Asparagus|Broccoli|Lettuce|Salmon|Spinach": "Ocean Greens Bowl",
		"Broccoli|Carrot|Peas|Rice|Tofu": "Vegan Stir-Fry Bowl",
		"Bread|Lettuce|Salmon|Steak|Tofu": "Triple Protein Sandwich",
		"Carrot|Lettuce|Potato|Rice|Spinach": "Green Carb Bowl",
		"Asparagus|Carrot|Potato|Rice|Salmon": "Fisherman's Garden Bowl",
		...
		# Additional 6-ingredient special recipes
		"Asparagus|Broccoli|Carrot|Lettuce|Peas|Spinach": "Garden of Everything",
		"Bread|Chicken Breast|Lettuce|Salmon|Steak|Tofu": "Ultimate Protein Sandwich",
		"Broccoli|Carrot|Potato|Rice|Spinach|Tofu": "Vegan Power Bowl",
		"Asparagus|Chicken Breast|Lettuce|Rice|Salmon|Spinach": "Healthy Fusion Bowl",
		"Bread|Broccoli|Carrot|Peas|Potato|Steak": "Loaded Veggie Steak Sub",
		"Chicken Breast|Peas|Potato|Rice|Salmon|Steak": "Meat Lover's Special",
		"Asparagus|Broccoli|Carrot|Peas|Potato|Rice": "Complete Garden Bowl",
		"Broccoli|Lettuce|Peas|Spinach|Steak|Tofu": "Green Protein Plate",
		
		# ===== 7 INGREDIENTS =====
		# Near-ultimate recipes with absurdist flair
	...
	# SMART FALLBACK: Generate contextual name based on ingredients
	return _generate_contextual_recipe_name(sorted_ingredients)
