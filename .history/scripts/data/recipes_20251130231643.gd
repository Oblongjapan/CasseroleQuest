...
func combine_ingredients_from_names(ingredient_names: Array) -> IngredientModel:
	...
	for ing_name in ingredient_names:
		...
		if "+" in clean_name:
			...
			for part in combo_parts:
				...
				if IngredientsData.INGREDIENTS.has(part_clean):
					var stats = IngredientsData.INGREDIENTS[part_clean]
					combo_ingredients.append(IngredientModel.new(part_clean, stats[0], stats[1], stats[2]))
			# Combine them
			var combo = combine_ingredients(combo_ingredients, 8)  # Use max tier to avoid restrictions
			if combo:
				ingredients.append(combo)
		else:
			# Single ingredient - check if it exists
			if IngredientsData.INGREDIENTS.has(clean_name):
				var stats = IngredientsData.INGREDIENTS[clean_name]
				print("[RecipesData] Found ingredient '%s' with stats: %s" % [clean_name, stats])
				ingredients.append(IngredientModel.new(clean_name, stats[0], stats[1], stats[2]))
			else:
	...
	if ingredients.is_empty():
		...
		print("[RecipesData] Available ingredients: %s" % IngredientsData.INGREDIENTS.keys())
		return null
