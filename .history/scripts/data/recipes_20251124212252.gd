...
## Water = sum, Heat Resistance = average, Volatility = SUM (more ingredients = more chaos!)
...
func combine_ingredients(ingredients: Array[IngredientModel], current_tier: int = 1) -> IngredientModel:
	...
	# Volatility is now SUMMED, not averaged - more ingredients = more chaos!
	var final_volatility = total_volatility
	
	print("[RecipesData] Totals: Water=%d, RST total=%d, Vol total=%d" % [total_water, total_heat_resistance, total_volatility])
	print("[RecipesData] Final stats: RST (avg)=%d, Vol (sum)=%d" % [avg_heat_resistance, final_volatility])
