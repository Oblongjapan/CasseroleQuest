...
func _check_and_create_recipe() -> void:
	...
	# First check if it matches a predefined recipe
	var recipe = RecipesData.find_recipe_by_ingredients(ingredient_names)
	if recipe:
		print("[Main] Predefined recipe created: %s" % recipe.name)
		...
		# No predefined recipe - create dynamic combo using tier system
		var combined_ingredient = RecipesData.combine_ingredients(current_round_ingredients)
		if combined_ingredient:
			print("[Main] Dynamic combo created: %s" % combined_ingredient.name)
			
			# Generate a unique recipe ID for tracking
			var recipe_id = "combo_" + "_".join(ingredient_names).replace(" ", "_").to_lower()
