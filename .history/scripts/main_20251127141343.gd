...
func _ready():
	...
	# Connect radio overlay button
	if radio_overlay:
		radio_overlay.pressed.connect(_on_radio_pressed)
		print("[Main] Radio overlay connected")
	...
	if music_player:
		...
		# Set pitch scale to 1.5x for faster gameplay feel
		...
		print("[Main] Music pitch_scale set to 1.5x and looping enabled")
		
		# Connect to finished signal to handle manual looping if needed
		music_player.finished.connect(_on_music_finished)
		
		# Add a timer to periodically check and enforce pitch scaling (HTML5 workaround)
		var pitch_check_timer = Timer.new()
		pitch_check_timer.name = "PitchCheckTimer"
		pitch_check_timer.wait_time = 0.5  # Check every half second
		pitch_check_timer.autostart = true
		pitch_check_timer.timeout.connect(_enforce_pitch_scale)
		add_child(pitch_check_timer)
		print("[Main] Pitch check timer added")
...
func _process(delta: float) -> void:
	...
	# Ensure music pitch stays at correct value based on radio state (HTML5 compatibility)
	if music_player and radio_overlay:
		var pitch_enabled = radio_overlay.get_meta("_pitch_enabled", true)
		var target_pitch = 1.5 if pitch_enabled else 1.0
		if music_player.pitch_scale != target_pitch:
...
func _on_radio_pressed() -> void:
	if not radio_overlay or not music_player:
		return
	
	# Toggle pitch state
	var pitch_enabled = radio_overlay.get_meta("_pitch_enabled", true)
	pitch_enabled = not pitch_enabled
	radio_overlay.set_meta("_pitch_enabled", pitch_enabled)
	...
	var animation_player = radio_overlay.get_node_or_null("AnimationPlayer")
	if animation_player:
		animation_player.speed_scale = target_pitch
	
	print("[Main] Radio toggled - pitch_enabled: %s, pitch: %.1fx" % [pitch_enabled, target_pitch])

## Periodically enforce pitch scale (HTML5 compatibility workaround)
func _enforce_pitch_scale() -> void:
	if not music_player or not radio_overlay:
		return
	
	var pitch_enabled = radio_overlay.get_meta("_pitch_enabled", true)
	var target_pitch = 1.5 if pitch_enabled else 1.0
	
	if abs(music_player.pitch_scale - target_pitch) > 0.01:  # Small tolerance for floating point
		print("[Main] CORRECTING pitch_scale from %.2f to %.2f" % [music_player.pitch_scale, target_pitch])
		music_player.pitch_scale = target_pitch
...
## Handle music looping and ensure pitch stays correct (HTML5 compatibility)
func _on_music_finished() -> void:
	if music_player and radio_overlay:
		var pitch_enabled = radio_overlay.get_meta("_pitch_enabled", true)
		var target_pitch = 1.5 if pitch_enabled else 1.0
		print("[Main] Music finished, re-applying pitch_scale: %.1fx" % target_pitch)
		music_player.pitch_scale = target_pitch
		# Note: stream.loop = true should handle automatic restart,
		# but we ensure pitch_scale is correct when it does
...
func _on_sample_selected(index: int) -> void:
	...
	# Add to deck
	fridge_manager.add_ingredient_to_deck(selected_ingredient)
