...
func _apply_shop_music_filter() -> void:
	...
	# Add filter to the music bus
	AudioServer.add_bus_effect(music_bus_index, low_pass_filter)
	print("[Main] Applied low pass filter to music (cutoff: 800 Hz)")
...
func _remove_shop_music_filter() -> void:
	...
	# Find and remove the low pass filter from the bus
	...
	for i in range(effect_count):
		...
		if effect is AudioEffectLowPassFilter:
			...
			print("[Main] Removed low pass filter from music")
			break
