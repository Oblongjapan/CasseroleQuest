...
@onready var moisture_label: Label = $UI/MoistureLabel/MoistureLabel
...
func _update_moisture_display() -> void:
	...
	if ingredient_overlay_1 and ingredient_overlay_1.current_ingredient_name != "":
		...
		if ing1:
			...
			max_moisture += 100  # Each ingredient contributes to max
...
func _find_ingredient_by_name(ingredient_name: String) -> IngredientModel:
	if hand_selector and hand_selector.has_method("get_selected_ingredients"):
		var selected = hand_selector.get_selected_ingredients()
		for ing in selected:
